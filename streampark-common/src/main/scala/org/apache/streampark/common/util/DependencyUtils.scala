...
object DependencyUtils {
  ...
  def resolveMavenDependencies(
    ...
    val ivySettings = ivySettingsPath match {
      case null => buildIvySettings(Option(repositories), Option(ivyRepoPath), outCallback)
      case path => loadIvySettings(path, Option(repositories), Option(ivyRepoPath), outCallback)
    }
    resolveMavenCoordinates(pom, ivySettings, outCallback)
  }
   ...
   */
  private[this] case class MavenCoordinate(groupId: String, artifactId: String, version: String) {
    override def toString: String = s"$groupId:$artifactId:$version"
  }

  /**
   * Extracts maven coordinates from a comma-delimited string. Coordinates should be provided in the
   * format `groupId:artifactId:version` or `groupId/artifactId:version`.
   *
   * @param coordinates
   ...
   */
  private[this] def extractMavenCoordinates(coordinates: String): Seq[MavenCoordinate] = {
   ...
   */
  private[this] def createRepoResolvers(defaultIvyUserDir: File): ChainResolver = {
    // We need a chain resolver if we want to check multiple repositories
    val cr = new ChainResolver
    cr.setName("streampark-list")
   ...
   * @return
   *   a comma-delimited list of paths for the dependencies
   */
  private[this] def resolveDependencyPaths(
  ...
  private[this] def addDependenciesToIvy(
  ...
  private[this] def addExclusionRules(
   ...
   */
  private[this] def buildIvySettings(
   ...
   *   An IvySettings object
   */
  private[this] def loadIvySettings(
      settingsFile: String,
      remoteRepos: Option[String],
      ivyPath: Option[String],
      outCallback: Consumer[String]): IvySettings = {
    ...
    require(
      file.exists(),
      s"[StreamPark] DependencyUtils.loadIvySettings: Ivy settings file $file does not exist")
    require(
      file.isFile,
      s"[StreamPark] DependencyUtils.loadIvySettings: Ivy settings file $file is not a normal file")
    val ivySettings: IvySettings = new IvySettings
    try {
      ...
      case e @ (_: IOException | _: ParseException) =>
        throw new RuntimeException(
          s"DependencyUtils.loadIvySettings: Failed when loading Ivy settings from $settingsFile",
          e)
    }
    processIvyPathArg(ivySettings, ivyPath)
    processRemoteRepoArg(ivySettings, remoteRepos, outCallback)
  ...
  private[this] def processIvyPathArg(ivySettings: IvySettings, ivyPath: Option[String]): Unit = {
    ivyPath.filterNot(_.trim.isEmpty).foreach {
      alternateIvyDir =>
  ...
  private[this] def processRemoteRepoArg(
      ivySettings: IvySettings,
      remoteRepos: Option[String],
      outCallback: Consumer[String]): Unit = {
    remoteRepos.filterNot(_.trim.isEmpty).map(_.split(",")).foreach {
      repositoryList =>
        ...
        repositoryList.zipWithIndex.foreach {
          case (repo, i) =>
  ...
  private[this] def getModuleDescriptor: DefaultModuleDescriptor =
  ...
  private[this] def clearIvyResolutionFiles(
    ...
    currentResolutionFiles.foreach {
      filename => new File(ivySettings.getDefaultCache, filename).delete()
    }
  }
   ...
   * @return
   *   The comma-delimited path to the jars of the given maven artifacts including their transitive
   *   dependencies
   */
  @throws[Exception]
  private[this] def resolveMavenCoordinates(
      pom: String,
      ivySettings: IvySettings,
      outCallback: Consumer[String],
      isTest: Boolean = false): List[String] = {
    if (Utils.isEmpty(pom)) List.empty[String]
    else {
      try {
        setDefaultLogger(outCallback)
        // To prevent ivy from logging to system out
        val artifacts = extractMavenCoordinates(pom)
        ...
        // md.addExcludeRule(createExclusion("*:*:*", ivySettings, ivyConfName))
  ...
  private[this] def createExclusion(
      coords: String,
      ivySettings: IvySettings,
