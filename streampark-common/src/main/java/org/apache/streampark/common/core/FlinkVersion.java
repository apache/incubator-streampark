...
/**
 * Discovers Flink and Scala versions and runtime libraries from a local Flink installation.
 *
 * <p>Distribution JAR names are preferred because they avoid starting a separate Flink process.
 * The command-line frontend is used only when the distribution name does not expose a version.
 */
public class FlinkVersion implements Serializable {

    private static final long serialVersionUID = 1L;

    private static final Logger LOG = StreamParkLoggerFactory.loggerFactory().getLogger(FlinkVersion.class.getName());

    private static final Pattern FLINK_VER_PATTERN = Pattern.compile("^(\\d+\\.\\d+)(?:\\..*)?$");

    private static final Pattern FLINK_VERSION_PATTERN = Pattern.compile("^Version: ([^,]*), Commit ID: (.*)$");
    ...
    /**
     * Creates a lazily evaluated descriptor for a Flink installation.
     *
     * @param flinkHome readable local Flink installation directory
     */
    ...
    private Optional<String[]> parseFromDistJar() {
        ...
        if (dashMatcher.matches()) {
            ...
            LOG.info(
                ...
                parsedVersion,
                "2.12");
            return Optional.of(new String[]{parsedVersion, "2.12"});
        }
