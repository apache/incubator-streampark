...
package com.streamxhub.streamx.flink.connector.clickhouse.scala.internal

import com.streamxhub.streamx.common.enums.ApiType
import com.streamxhub.streamx.common.enums.ApiType.ApiType
import com.streamxhub.streamx.common.util.Logger
import com.streamxhub.streamx.flink.connector.clickhouse.scala.conf.ClickHouseConfig
import com.streamxhub.streamx.flink.connector.clickhouse.scala.conf.ClickHouseConfigConst.CLICKHOUSE_TARGET_TABLE
import com.streamxhub.streamx.flink.connector.clickhouse.scala.internal
import com.streamxhub.streamx.flink.connector.clickhouse.scala.util.PoJoConvertUtils.parsePojoToInsertValue
...
class AsyncClickHouseSinkFunction[T](apiType: ApiType = ApiType.scala, properties: Properties) extends RichSinkFunction[T] with Logger {
  ...
  private[this] var javaSqlFunc: PojoToStringFunction[T] = _
  ...
  //for JAVA
  def this(properties: Properties,
           javaSqlFunc: PojoToStringFunction[T]) = {
  ...
  override def invoke(value: T): Unit = {
    val csv = (javaSqlFunc, scalaSqlFunc) match {
      ...
      case _ => apiType match {
        case ApiType.java => javaSqlFunc.translateToString(value)
