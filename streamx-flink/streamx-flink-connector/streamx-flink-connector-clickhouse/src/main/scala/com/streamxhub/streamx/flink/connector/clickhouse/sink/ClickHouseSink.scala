...
class ClickHouseSink(@(transient@param) ctx: StreamingContext,
   ...
   * @return
   */
  def sink[T](stream: JavaDataStream[T], toSQLFn: SQLFromFunction[T]): DataStreamSink[T] = {
    checkParameters(stream)
    val sinkFun = new AsyncClickHouseSinkFunction[T](prop, toSQLFn)
    val sink = stream.addSink(sinkFun)
    afterSink(sink, parallelism, name, uid)
   ...
   * @return
   */
  def syncSink[T](stream: JavaDataStream[T], sqlFromFn: SQLFromFunction[T]): DataStreamSink[T] = {
