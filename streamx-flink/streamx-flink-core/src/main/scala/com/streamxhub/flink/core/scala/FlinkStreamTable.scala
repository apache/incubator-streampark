...
import org.apache.flink.streaming.api.environment.{
  CheckpointConfig,
  StreamExecutionEnvironment => JavaStreamExecutionEnvironment
}
...
class StreamTableContext(
  ...
  /**
    ...
    */
  def this(
      args: (ParameterTool, StreamExecutionEnvironment, StreamTableEnvironment)
  ...
  def this(args: StreamEnvConfig) =
    this(FlinkTableInitializer.initJavaStreamTable(args))
  ...
  def $getCachedFiles
      : JavaList[tuple.Tuple2[String, DistributedCache.DistributedCacheEntry]] =
  ...
  ): StreamExecutionEnvironment = this.streamEnv.setRuntimeMode(executionMode)
  ...
  def $setRestartStrategy(
      restartStrategyConfiguration: RestartStrategies.RestartStrategyConfiguration
  ): Unit = this.streamEnv.setRestartStrategy(restartStrategyConfiguration)
  ...
  def $addDefaultKryoSerializer[T <: Serializer[_] with Serializable](
      `type`: Class[_],
      serializer: T
  ): Unit = this.streamEnv.addDefaultKryoSerializer(`type`, serializer)

  def $addDefaultKryoSerializer(
      `type`: Class[_],
      serializerClass: Class[_ <: Serializer[_]]
  ): Unit = this.streamEnv.addDefaultKryoSerializer(`type`, serializerClass)

  def $registerTypeWithKryoSerializer[T <: Serializer[_] with Serializable](
      clazz: Class[_],
      serializer: T
  ): Unit = this.streamEnv.registerTypeWithKryoSerializer(clazz, serializer)
  ...
  ): Unit = this.streamEnv.configure(configuration, classLoader)

  def $fromSequence(from: Long, to: Long): DataStream[Long] =
    this.streamEnv.fromSequence(from, to)

  def $fromElements[T](data: T*)(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.fromElements(data: _*)

  def $fromCollection[T](data: Seq[T])(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.fromCollection(data)

  def $fromCollection[T](data: Iterator[T])(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.fromCollection(data)

  def $fromParallelCollection[T](data: SplittableIterator[T])(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.fromParallelCollection(data)
  ...
  def $readFile[T](inputFormat: FileInputFormat[T], filePath: String)(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.readFile(inputFormat, filePath)
  ...
  def $createInput[T](inputFormat: InputFormat[T, _])(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.createInput(inputFormat)

  def $addSource[T](function: SourceFunction[T])(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.addSource(function)

  def $addSource[T](function: SourceFunction.SourceContext[T] => Unit)(
      implicit info: TypeInformation[T]
  ): DataStream[T] = this.streamEnv.addSource(function)
  ...
  def $getStreamGraph(
      jobName: String,
      clearTransformations: Boolean
  ): StreamGraph = this.streamEnv.getStreamGraph(jobName, clearTransformations)
  ...
  def $registerCachedFile(
      filePath: String,
      name: String,
      executable: Boolean
  ): Unit = this.streamEnv.registerCachedFile(filePath, name, executable)
  ...
  override def fromDataStream[T](
      dataStream: DataStream[T],
      fields: Expression*
  ): Table = tableEnv.fromDataStream(dataStream, fields: _*)

  override def createTemporaryView[T](
      path: String,
      dataStream: DataStream[T]
  ): Unit = tableEnv.createTemporaryView(path, dataStream)

  override def createTemporaryView[T](
      path: String,
      dataStream: DataStream[T],
      fields: Expression*
  ): Unit = tableEnv.createTemporaryView(path, dataStream, fields: _*)

  override def toAppendStream[T](table: Table)(
      implicit info: TypeInformation[T]
  ): DataStream[T] = tableEnv.toAppendStream(table)

  override def toRetractStream[T](table: Table)(
      implicit info: TypeInformation[T]
  ): DataStream[(Boolean, T)] = tableEnv.toRetractStream(table)
  ...
  @Deprecated override def registerFunction[T](
      name: String,
      tf: TableFunction[T]
  )(implicit info: TypeInformation[T]): Unit =
    tableEnv.registerFunction(name, tf)

  @Deprecated override def registerFunction[T, ACC](
      name: String,
      f: AggregateFunction[T, ACC]
  )(implicit info1: TypeInformation[T], info2: TypeInformation[ACC]): Unit =
    tableEnv.registerFunction(name, f)
  ...
  @Deprecated override def registerDataStream[T](
      name: String,
      dataStream: DataStream[T]
  ): Unit = tableEnv.registerDataStream(name, dataStream)

  @Deprecated override def registerDataStream[T](
      name: String,
      dataStream: DataStream[T],
      fields: Expression*
  ): Unit = tableEnv.registerDataStream(name, dataStream, fields: _*)
  ...
  ): StreamTableDescriptor = tableEnv.connect(connectorDescriptor)
...
trait FlinkStreamTable extends Logger {
  ...
  final implicit def tableExt(table: Table): TableExt.Table =
    new TableExt.Table(table)

  final implicit def tableConversions(table: Table): TableExt.TableConversions =
  ...
  def main(args: Array[String]): Unit = {
    ...
    context = new StreamTableContext(
      FlinkTableInitializer.initStreamTable(args, config)
    )
    beforeStart(context)
    handle(context)
