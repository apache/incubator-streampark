...
object HttpSink {
  ...
  def apply(
      ...
      name: String = null,
      uid: String = null
  ): HttpSink = new HttpSink(ctx, header, parallelism, name, uid)
...
class HttpSink(
  ...
  def getSink(stream: DataStream[String]): DataStreamSink[String] =
    sink(stream, HttpGet.METHOD_NAME)

  def postSink(stream: DataStream[String]): DataStreamSink[String] =
    sink(stream, HttpPost.METHOD_NAME)

  def patchSink(stream: DataStream[String]): DataStreamSink[String] =
    sink(stream, HttpPatch.METHOD_NAME)

  def putSink(stream: DataStream[String]): DataStreamSink[String] =
    sink(stream, HttpPut.METHOD_NAME)

  def deleteSink(stream: DataStream[String]): DataStreamSink[String] =
    sink(stream, HttpDelete.METHOD_NAME)

  def optionsSink(stream: DataStream[String]): DataStreamSink[String] =
    sink(stream, HttpOptions.METHOD_NAME)

  def traceSink(stream: DataStream[String]): DataStreamSink[String] =
  ...
  private[this] def sink(
    ...
    val params = ctx.parameter.toMap
      .filter(_._1.startsWith(HTTP_SINK_PREFIX))
      .map(x => x._1.drop(HTTP_SINK_PREFIX.length + 1) -> x._2)
  ...
  override def open(config: Configuration): Unit = {
    if (!Lock.initialized) {
      ...
      if (!Lock.initialized) {
        ...
        sinkBuffer =
          SinkBuffer(httpSinkWriter, thresholdConf.delayTime, bufferSize, table)
...
case class HttpSinkWriter(
  ...
  private val threadFactory: ThreadFactory =
    ThreadUtils.threadFactory("HttpSink-writer")
...
case class HttpWriterTask(
  ...
  val failoverWriter: FailoverWriter = new FailoverWriter(
    thresholdConf.storageType,
    thresholdConf.getFailoverConfig
  )
    ...
    try {
      ...
      while (isWorking || queue.nonEmpty) {
        ...
        if (req != null) {
          ...
          val sinkRequest =
            SinkRequest(List(url), req.table, req.attemptCounter)
          ...
          if (req.attemptCounter > 0) {
            logInfo(
              s"get retry url from queue,attemptCounter:${req.attemptCounter}"
            )
          }
        }
      }
    ...
    try {
      if (sinkRequest.attemptCounter >= thresholdConf.maxRetries) {
        failoverWriter.write(
          sinkRequest.copy(
            records = sinkRequest.records.map(_.replaceFirst("^[A-Z]+///", ""))
          )
        )
        logWarn(
      ...
      case e: Exception =>
        new RuntimeException(s"[StreamX] handleFailedResponse,error:$e")
    }
