...
object ClickHouseSink {
  ...
  def apply(
      ...
      name: String = null,
      uid: String = null
  )(implicit alias: String = ""): ClickHouseSink =
...
class ClickHouseSink(
  ...
  val prop =
    ConfigUtils.getConf(ctx.parameter.toMap, CLICKHOUSE_SINK_PREFIX)(alias)
  ...
  /**
    ...
    * @return
    */
  def sink[T](stream: DataStream[T], dbAndTable: String)(
      implicit toCSVFun: T => String = null
  ): DataStreamSink[T] = {
  ...
  def syncSink[T](
      stream: DataStream[T]
  )(implicit toSQLFn: T => String): DataStreamSink[T] = {
...
class AsyncClickHouseSinkFunction[T](properties: Properties)(
    implicit toCSVFun: T => String = null
      ...
      Lock.lock.synchronized {
        if (!Lock.initialized) {
          ...
          sinkBuffer = SinkBuffer(
            clickHouseWriter,
            clickHouseConf.delayTime,
            clickHouseConf.bufferSize,
            table
          )
  ...
  override def invoke(value: T): Unit = {
    ...
    try {
      ...
      case e: Exception =>
        logError(
          s"""Error while sending data to Clickhouse, record = $csv,error:$e"""
        )
...
class AsyncClickHouseOutputFormat[T: TypeInformation](
    implicit prop: Properties,
    toSQlFun: T => String = null
...
class ClickHouseSinkFunction[T](config: Properties)(
    implicit toSQLFn: T => String = null
) extends RichSinkFunction[T]
    with Logger {
  ...
  override def open(parameters: Configuration): Unit = {
    ...
    val user: String =
      Try(config.remove(KEY_JDBC_USER).toString).getOrElse(null)
    val driver: String =
      Try(config.remove(KEY_JDBC_DRIVER).toString).getOrElse(null)
    ...
    config.foreach(x => {
      val field = Try(Option(properties.getClass.getDeclaredField(x._1)))
        .getOrElse(None) match {
        case None =>
          val boolField =
            s"is${x._1.substring(0, 1).toUpperCase}${x._1.substring(1)}"
          Try(Option(properties.getClass.getDeclaredField(boolField)))
            .getOrElse(None) match {
            case Some(x) => x
            case None =>
...
class ClickHouseOutputFormat[T: TypeInformation](
    implicit prop: Properties,
    toSQlFun: T => String
) extends RichOutputFormat[T]
    with Logger {
...
class ClickHouseConfig(parameters: Properties)
    extends ThresholdConf(parameters) {
  var currentHostId: Int = 0
  val credentials: String = (
    parameters.getProperty(KEY_JDBC_USER),
    parameters.getProperty(KEY_JDBC_PASSWORD)
  ) match {
    case (null, null) => null
    case (u, p)       => new String(Base64.getEncoder.encode(s"$u:$p".getBytes))
  }
  val jdbcUrls: java.util.List[String] = parameters
    .getOrElse(KEY_JDBC_URL, "")
    .split(SIGN_COMMA)
    .filter(_.nonEmpty)
...
case class ClickHouseSinkWriter(clickHouseConfig: ClickHouseConfig)
    extends SinkWriter
    with Logger {
  private val callbackServiceFactory =
    ThreadUtils.threadFactory("ClickHouse-writer-callback-executor")
  private val threadFactory: ThreadFactory =
    ThreadUtils.threadFactory("ClickHouse-writer")
  ...
  var tasks: ListBuffer[ClickHouseWriterTask] =
    ListBuffer[ClickHouseWriterTask]()
...
case class ClickHouseWriterTask(
  ...
  val failoverWriter: FailoverWriter = new FailoverWriter(
    clickHouseConf.storageType,
    clickHouseConf.getFailoverConfig
  )
  ...
  def send(sinkRequest: SinkRequest): Unit = {
    ...
    logDebug(
      s"Ready to load data to ${sinkRequest.table}, size = ${sinkRequest.size}"
    )
  ...
  def buildRequest(sinkRequest: SinkRequest): Request = {
    val query =
      s"INSERT INTO ${sinkRequest.table} VALUES ${sinkRequest.records.mkString(",")}"
    ...
    if (clickHouseConf.credentials != null) {
      builder.setHeader(
        ...
        "Basic " + clickHouseConf.credentials
      )
    }
    builder.build
  }
  ...
  def respCallback(
      whenResponse: ListenableFuture[Response],
