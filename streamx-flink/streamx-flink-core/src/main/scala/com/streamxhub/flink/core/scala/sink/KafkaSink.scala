...
object KafkaSink {
  def apply(
      ...
      name: String = null,
      uid: String = null
  ): KafkaSink = new KafkaSink(ctx, property, parallelism, name, uid)
}
...
class KafkaSink(
  ...
  def sink[T](
      ...
      partitioner: FlinkKafkaPartitioner[T] =
        new KafkaEqualityPartitioner[T](ctx.getParallelism)
  ): DataStreamSink[T] = {
    ...
    val producer = {
      ...
      /**
        ...
        */
      val semantic = Try(
        Some(prop.remove(ConfigConst.KEY_KAFKA_SEMANTIC).toString.toUpperCase)
        ...
        case _ =>
          throw new IllegalArgumentException(
            "[StreamX] kafka.sink semantic error,muse be (AT_LEAST_ONCE|EXACTLY_ONCE|NONE) "
          )
      }
      ...
      val customPartitioner = partitioner match {
        case null =>
          Optional
            .ofNullable(null)
            .asInstanceOf[Optional[FlinkKafkaPartitioner[T]]]
...
/**
  ...
  * @tparam T
  */
class KafkaEqualityPartitioner[T](parallelism: Int)
  ...
  override def open(parallelInstanceId: Int, parallelInstances: Int): Unit = {
    ...
    require(
      ...
      "[StreamX] KafkaEqualityPartitioner:Id of this subtask cannot be negative,Number of subtasks must be larger than 0."
    )
  ...
  override def partition(
    ...
    require(
      ...
      "[StreamX] KafkaEqualityPartitioner:Partitions of the target topic is empty."
    )
  ...
  override def equals(o: Any): Boolean =
    this == o || o.isInstanceOf[KafkaEqualityPartitioner[T]]
