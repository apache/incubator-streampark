...
object HBaseRequest {
  ...
  ): HBaseRequest[T] = new HBaseRequest[T](stream, property)
...
class HBaseRequest[T: TypeInformation](
  ...
  def requestOrdered[R: TypeInformation](
      ...
      timeout: Long = 1000,
      capacity: Int = 10
  )(implicit prop: Properties): DataStream[R] = {
    Utils.copyProperties(property, prop)
    val async =
      new HBaseAsyncFunction[T, R](prop, queryFunc, resultFunc, capacity)
...
class HBaseAsyncFunction[T: TypeInformation, R: TypeInformation](
  ...
  override def asyncInvoke(
      input: T,
      resultFuture: async.ResultFuture[R]
  ): Unit = {
    CompletableFuture
      .supplyAsync(
        new Supplier[ResultScanner]() {
          override def get(): ResultScanner = {
            ...
            require(
              ...
              "[StreamX] HBaseRequest query and query's attr table must be not null "
            )
