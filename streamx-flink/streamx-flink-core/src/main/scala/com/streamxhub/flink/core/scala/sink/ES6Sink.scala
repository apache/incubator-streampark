...
object ES6Sink {
  ...
  def apply(
      ...
      name: String = null,
      uid: String = null
  ): ES6Sink = new ES6Sink(ctx, property, parallelism, name, uid)
...
class ES6Sink(
  ...
  def sink[T](
    ...
    if (restClientFactory != null) {
      require(
        restClientFactory.isInstanceOf[RestClientFactory],
        "restClientFactory error: must be RestClientFactory instance"
      )
    }
    ...
    val httpHosts = shortConfig
      .getOrElse(KEY_HOST, SIGN_EMPTY)
      .split(SIGN_COMMA)
      .map(x => {
    ...
    require(
      httpHosts.nonEmpty,
      "elasticsearch config error,please check, e.g: sink.es.host=$host1:$port1,$host2:$port2"
    )

    val sinkFunc: ElasticsearchSinkFunction[T] =
      new ElasticsearchSinkFunction[T] {
        ...
        ): Unit = requestIndexer.add(createIndexRequest(element))
      }

    val sinkBuilder =
      new ElasticsearchSink.Builder[T](httpHosts.toList, sinkFunc)
    // failureHandler
    ...
    if (restClientFactory == null) {
      ...
      sinkBuilder.setRestClientFactory(
        restClientFactory.asInstanceOf[RestClientFactory]
      )
    }

    def doConfig(param: (String, String)): Unit = param match {
      // parameter of sink.es.bulk.flush.max.actions
      case (CONFIG_KEY_BULK_FLUSH_MAX_ACTIONS, v) =>
        sinkBuilder.setBulkFlushMaxActions(v.toInt)
      // parameter of sink.es.bulk.flush.max.size.mb
      case (CONFIG_KEY_BULK_FLUSH_MAX_SIZE_MB, v) =>
        sinkBuilder.setBulkFlushMaxSizeMb(v.toInt)
      // parameter of sink.es.bulk.flush.interval.ms
      case (CONFIG_KEY_BULK_FLUSH_INTERVAL_MS, v) =>
        sinkBuilder.setBulkFlushInterval(v.toInt)
      // parameter of sink.es.bulk.flush.backoff.enable
      case (CONFIG_KEY_BULK_FLUSH_BACKOFF_ENABLE, v) =>
        sinkBuilder.setBulkFlushBackoff(v.toBoolean)
    ...
    property.forEach(new BiConsumer[Object, Object] {
      override def accept(k: Object, v: Object): Unit =
        doConfig(k.toString, v.toString)
    })
...
class RestClientFactoryImpl(val config: Map[String, String])
  ...
  override def configureRestClientBuilder(
    ...
    def configCallback(): RestClientBuilder = {
      ...
      val requestConfigCallback = new RestClientBuilder.RequestConfigCallback {
        override def customizeRequestConfig(
          ...
          config.foreach {
            case (KEY_ES_CONN_REQ_TIME_OUT, v) =>
              requestConfigBuilder.setConnectionRequestTimeout(v.toInt)
            case (KEY_ES_CONN_TIME_OUT, v) =>
              requestConfigBuilder.setConnectTimeout(v.toInt)
            case _ =>
          }
          //other config....
    ...
    def setHeader(): RestClientBuilder = {
      val contentType =
        config.getOrElse(KEY_ES_REST_CONTENT_TYPE, "application/json")
      val maxRetry = Try(config.get(KEY_ES_REST_MAX_RETRY).toString.toInt) match {
        case Success(value) => value
        case _ =>
          logWarn(
            s" config error: $KEY_ES_REST_MAX_RETRY is not set or invalid,use 10000 "
          )
          10000
      }
