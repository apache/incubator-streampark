...
import com.streamxhub.flink.core.scala.enums.{
  ApiType,
  RestartStrategy,
...
private[scala] class FlinkStreamingInitializer(
    args: Array[String],
    apiType: ApiType
) extends Logger {
  ...
  private[this] def readFlinkConf(config: String): Map[String, String] = {
    ...
    val map = config match {
      ...
      case _ =>
        ...
        require(
          configFile.exists(),
          s"[StreamX] Usage:flink.conf file $configFile is not found!!!"
        )
        extension match {
          case "properties" =>
  ...
  private[this] def initParameter(): ParameterTool = {
    ...
    val config = argsMap.get(KEY_FLINK_CONF(), null) match {
      case null | "" =>
        throw new ExceptionInInitializerError(
          "[StreamX] Usage:can't fond config,please set \"--flink.conf $path \" in main arguments"
        )
      case file => file
    }
    ...
    ParameterTool
      ...
      .mergeWith(ParameterTool.fromMap(configArgs))
      .mergeWith(argsMap)
  }
  ...
  def initStreamEnv(): Unit = {
    ...
    Try(parameter.get(KEY_FLINK_PARALLELISM).toInt).getOrElse {
      Try(parameter.get(CoreOptions.DEFAULT_PARALLELISM.key()).toInt)
        .getOrElse(CoreOptions.DEFAULT_PARALLELISM.defaultValue().toInt)
    } match {
      case p if p > 0 => localStreamEnv.setParallelism(p)
      case _ =>
        throw new IllegalArgumentException(
          "[StreamX] parallelism muse be > 0. "
        )
    }
    val timeCharacteristic = Try(
      TimeCharacteristic
        .valueOf(parameter.get(KEY_FLINK_WATERMARK_TIME_CHARACTERISTIC))
    ).getOrElse(TimeCharacteristic.EventTime)
    val interval =
  ...
  private[this] def restartStrategy(): Unit = {
    val strategy = Try(
      RestartStrategy.byName(parameter.get(KEY_FLINK_RESTART_STRATEGY))
    ).getOrElse(null)
    strategy match {
      case RestartStrategy.`failure-rate` =>
        /**
          ...
          */
        val interval = Try(
          parameter.get(KEY_FLINK_RESTART_FAILURE_PER_INTERVAL).toInt
        ).getOrElse(3)

        val rateInterval = DateUtils.getTimeUnit(
          Try(parameter.get(KEY_FLINK_RESTART_FAILURE_RATE_INTERVAL))
            .getOrElse(null),
          (5, TimeUnit.MINUTES)
        )

        val delay = DateUtils.getTimeUnit(
          Try(parameter.get(KEY_FLINK_RESTART_FAILURE_RATE_DELAY))
            .getOrElse(null)
        )

        streamEnvironment.getConfig.setRestartStrategy(
      ...
      case RestartStrategy.`fixed-delay` =>
        /**
          ...
          */
        val attempts =
          Try(parameter.get(KEY_FLINK_RESTART_ATTEMPTS).toInt).getOrElse(3)
        val delay = DateUtils.getTimeUnit(
          Try(parameter.get(KEY_FLINK_RESTART_DELAY)).getOrElse(null)
        )
        ...
        streamEnvironment.getConfig.setRestartStrategy(
          RestartStrategies
            .fixedDelayRestart(attempts, Time.of(delay._1, delay._2))
        )
      ...
      case null =>
        logInfo("RestartStrategy not set,use default from $flink_conf")
    }
  }

  private[this] def checkpoint(): Unit = {
    //checkPoint,从配置文件读取是否开启checkpoint,默认不启用.
    val enableCheckpoint = Try(
      parameter.get(KEY_FLINK_CHECKPOINTS_ENABLE).toBoolean
    ).getOrElse(false)
    ...
    val cpMode = Try(
      CheckpointingMode.valueOf(parameter.get(KEY_FLINK_CHECKPOINTS_MODE))
    ).getOrElse(CheckpointingMode.EXACTLY_ONCE)
    val cpCleanUp = Try(
      ExternalizedCheckpointCleanup
        .valueOf(parameter.get(KEY_FLINK_CHECKPOINTS_CLEANUP))
    ).getOrElse(ExternalizedCheckpointCleanup.RETAIN_ON_CANCELLATION)
    val cpTimeout = Try(parameter.get(KEY_FLINK_CHECKPOINTS_TIMEOUT).toLong)
      .getOrElse(CheckpointConfig.DEFAULT_TIMEOUT)
    val cpMaxConcurrent = Try(
      parameter.get(KEY_FLINK_CHECKPOINTS_MAX_CONCURRENT).toInt
    ).getOrElse(CheckpointConfig.DEFAULT_MAX_CONCURRENT_CHECKPOINTS)
    val cpMinPauseBetween = Try(
      parameter.get(KEY_FLINK_CHECKPOINTS_MIN_PAUSEBETWEEN).toLong
    ).getOrElse(CheckpointConfig.DEFAULT_MIN_PAUSE_BETWEEN_CHECKPOINTS)
    val unaligned = Try(
      parameter.get(KEY_FLINK_CHECKPOINTS_UNALIGNED).toBoolean
    ).getOrElse(false)
    ...
    if (stateBackend != null) {
      val cpDir =
        if (stateBackend == XStateBackend.jobmanager) null
        else {
          ...
          parameter.get(KEY_FLINK_STATE_CHECKPOINTS_DIR, null) match {
            ...
            case null =>
              logWarn(
                "can't found flink.checkpoints.dir from properties,now try found from flink-conf.yaml"
              )
              val flinkConf = {
                ...
                val flinkHome = parameter.get(KEY_FLINK_HOME(), null) match {
                  case null | "" =>
                    logInfo(
                      "--flink.home is undefined,now try found from flink-conf.yaml on System env."
                    )
                    val flinkHome = System.getenv("FLINK_HOME")
                    require(
                      flinkHome != null,
                      "[StreamX] FLINK_HOME is not defined in your system."
                    )
                    flinkHome
                  case file => file
                }
              ...
              require(
                dir != null,
                s"[StreamX] can't found flink.checkpoints.dir from $flinkConf "
              )
              logInfo(
                s"stat.backend: flink.checkpoints.dir found in flink-conf.yaml,$dir"
              )
              dir
            case dir =>
              logInfo(
                s"stat.backend: flink.checkpoints.dir found in properties,$dir"
              )
              dir
          }
        }
      ...
      stateBackend match {
        ...
        case XStateBackend.jobmanager =>
          ...
          val maxMemorySize = Try(
            parameter.get(KEY_FLINK_STATE_BACKEND_MEMORY).toInt
          ).getOrElse(MemoryStateBackend.DEFAULT_MAX_STATE_SIZE)
          val async = Try(
            parameter.get(KEY_FLINK_STATE_BACKEND_ASYNC).toBoolean
          ).getOrElse(false)
        ...
        case XStateBackend.rocksdb =>
          ...
          // 默认开启增量.
          val incremental = Try(
            parameter.get(KEY_FLINK_STATE_BACKEND_INCREMENTAL).toBoolean
          ).getOrElse(true)
          ...
          val skipKey = List(
            KEY_FLINK_STATE_BACKEND_ASYNC,
            KEY_FLINK_STATE_BACKEND_INCREMENTAL,
            KEY_FLINK_STATE_BACKEND_MEMORY,
            KEY_FLINK_STATE_ROCKSDB
          )
          parameter.getProperties
            .filter(_._1.startsWith(KEY_FLINK_STATE_ROCKSDB))
            .filterNot(x => skipKey.contains(x._1))
        ...
        case _ =>
          logError(
            "usage error!!! stat.backend must be (jobmanager|filesystem|rocksdb)"
          )
      }
    }
  }
...
class StreamEnvConfig(
    ...
    val conf: StreamEnvConfigFunction
)
