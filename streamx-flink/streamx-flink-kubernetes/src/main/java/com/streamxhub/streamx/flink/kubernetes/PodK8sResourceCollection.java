...
public class PodK8sResourceCollection implements K8sResourceCollection<Pod> {
    ...
    public Iterable<Pod> listResources() {
        try {
            PodList resourceList = this.client.pods().inNamespace(this.namespace).withLabel(this.labelSelector).list();
