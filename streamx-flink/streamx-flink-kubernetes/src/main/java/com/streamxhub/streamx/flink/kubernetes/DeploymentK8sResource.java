...
public class DeploymentK8sResource implements PatchableK8SResourceWithPods<Deployment> {
    ...
    public DeploymentK8sResource(Config config, String namespace, Deployment resource, DeletionPropagationPolicy deletionPolicy) {
        ...
        this.pods = new PodKubernetesResourceCollection(config, namespace, Optional.ofNullable(resource.getSpec()).map((spec) -> spec.getTemplate().getMetadata()).map(ObjectMeta::getLabels).orElseThrow(() -> new IllegalArgumentException("Must provide Pod template labels")));
