...
doStart() {
    ...
    local allParam="$resource_params"
    if [ x"$dynamic_params" != x"" ]; then
        allParam="$resource_params $dynamic_params"
    fi
