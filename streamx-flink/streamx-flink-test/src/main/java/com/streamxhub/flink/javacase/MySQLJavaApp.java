...
public class MySQLJavaApp {
  ...
  public static void main(String[] args) {
    ...
    // 读取MySQL数据源
    DataStream<LogBean> stream =
        new MySQLSource<LogBean>(context, prop)
            .getDataStream(
                (SQLQueryFunction<LogBean>)
                    lastOne -> {
                      Thread.sleep(1000);
                      if (lastOne == null) {
                        return "select * from orders where out_time> 2020-10-10 00:00:00 limit 10 ";
                      } else {
                        return "select * from orders where out_time> "
                            + lastOne.getOut_time()
                            + " limit 10 ";
                      }
                    },
                (SQLResultFunction<LogBean>)
                    map -> {
    ...
    new JdbcSink<LogBean>(context)
        ...
        .sql(
            bean ->
                String.format(
                    "insert into sink(name,value)value('%s','%s')",
                    bean.getCard_type(), bean.getControlid()))
        .sink(stream);
