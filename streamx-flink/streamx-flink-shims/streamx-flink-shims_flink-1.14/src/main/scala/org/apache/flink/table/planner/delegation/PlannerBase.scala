...
import org.apache.flink.table.operations.ddl.{CreateOperation, CreateTableOperation}
...
abstract class PlannerBase(
  ...
  override def explainLineage(operations: util.List[Operation]): String = {
    ...
    operations.foreach(operation => {
      operation match {
        case operation: CreateTableOperation =>
          val tableName = operation.getTableIdentifier.asSummaryString()
          val options = operation.getCatalogTable.getOptions.asScala.toMap
          val fieldList = ListBuffer[Map[String, String]]()
          operation.getCatalogTable.asInstanceOf[CatalogTableImpl].getSchema.getTableColumns.foreach(col =>
            fieldList += Map("name" -> col.getName, "type" -> col.getType.toString)
          )
    ...
    val str = Serialization.write(Map("tables" -> tableList, "relations" -> relationList))
    println(str)
    str
  }
