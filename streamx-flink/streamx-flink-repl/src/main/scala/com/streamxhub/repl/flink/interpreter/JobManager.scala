...
import com.streamxhub.common.util.{DateUtils, Logger}
...
class JobManager(var flinkWebUrl: String, var replacedFlinkWebUrl: String)
  ...
  private val jobProgressPollerMap =
    new ConcurrentHashMap[JobID, FlinkJobProgressPoller]
  ...
  def addJob(context: InterpreterContext, jobClient: JobClient): Unit = {
    ...
    val thread =
      new FlinkJobProgressPoller(flinkWebUrl, jobClient.getJobID, context)
    ...
    if (previousJobClient != null) {
      logWarn(
        s"There's another Job ${jobClient.getJobID} that is associated with paragraph ${paragraphId}"
      )
    }
  }
  ...
  def removeJob(paragraphId: String): Unit = {
    ...
    if (jobClient == null) {
      logWarn(
        "Unable to remove job, because no job is associated with paragraph: " + paragraphId
      )
      return
    }
  ...
  def getJobProgress(paragraphId: String): Int = {
    ...
    if (jobClient == null) {
      logWarn(
        "Unable to get job progress for paragraph: " + paragraphId + ", because no job is associated with this paragraph"
      )
      return 0
    }
    ...
    if (jobProgressPoller == null) {
      logWarn(
        "Unable to get job progress for paragraph: " + paragraphId + ", because no job progress is associated with this jobId: " + jobClient.getJobID
      )
      return 0
    }
  ...
  def cancelJob(context: InterpreterContext): Unit = {
    ...
    if (jobClient == null) {
      logWarn(
        s"Unable to remove Job from paragraph {} as no job associated to this paragraph ${context.getParagraphId}"
      )
      return
    }
    var cancelled = false
    try {
      val savePointDir =
        context.getLocalProperties.get(JobManager.SAVEPOINT_DIR)
      if (StringUtils.isBlank(savePointDir)) {
        logInfo(s"Trying to cancel job of paragraph ${context.getParagraphId}")
        jobClient.cancel
      } else {
        logInfo(
          s"Trying to stop job of paragraph ${context.getParagraphId} with save point dir: ${savePointDir}"
        )
        ...
        logInfo(
          s"Job ${jobClient.getJobID} of paragraph ${context.getParagraphId} is stopped with save point path: ${savePointPath}"
        )
      }
      cancelled = true
    } catch {
      case e: Exception =>
        val errorMessage = String.format(
          "Fail to cancel job %s that is associated " + "with paragraph %s",
          jobClient.getJobID,
      ...
      logInfo(
        "Cancelling is successful, remove the associated FlinkJobProgressPoller of paragraph: " + context.getParagraphId
      )
...
class FlinkJobProgressPoller(
    var flinkWebUrl: String,
    var jobId: JobID,
    var context: InterpreterContext
) extends Thread
    with Logger {

  this.isStreamingInsertInto =
    context.getLocalProperties.containsKey("flink.streaming.insert_into")
    ...
    breakable {
      while (!Thread.currentThread.isInterrupted && running.get) {
        ...
        try {
          ...
          rootNode =
            Unirest.get(flinkWebUrl + "/jobs/" + jobId.toString).asJson.getBody
          ...
          if (isStreamingInsertInto) {
            ...
            context.getAngularObjectRegistry.add(
              "duration",
              DateUtils.toRichTimeDuration(duration),
              context.getNoteId,
              context.getParagraphId
            )
          }
          ...
          rootNode = Unirest
            .get(flinkWebUrl + "/jobs/" + jobId.toString + "/checkpoints")
            .asJson
            .getBody
          if (rootNode.getObject.has("latest")) {
            val latestObject = rootNode.getObject.getJSONObject("latest")
            if (latestObject.has("completed") && latestObject
                  .get("completed")
                  .isInstanceOf[JSONObject]) {
