...
object FlinkShell extends Logger {
  ...
  @Internal def getClusterClient(
    ...
    val (effectiveConfig, clusterClient) = config.executionMode match {
      ...
      case ExecutionMode.YARN =>
        createYarnSessionCluster(config, flinkConfig, flinkShims)
      case _ =>
        throw new IllegalArgumentException(
          "please specify execution mode:[local | remote <host> <port> | yarn]"
        )
    }
    logInfo(
      s"Notebook connectionInfo:ExecutionMode:${config.executionMode},config:$effectiveConfig"
    )
  ...
  def getCliFrontend(effectiveConfig: Configuration, config: Config) = {
    ...
    new CliFrontend(
      effectiveConfig,
      CliFrontend.loadCustomCommandLines(effectiveConfig, configDir)
    )
  }

  private[this] def createYarnSessionCluster(
      config: Config,
      flinkConfig: Configuration,
      flinkShims: FlinkShims
  ) = {
    ...
    val (clusterConfig, clusterClient) = config.yarnConfig match {
      case Some(_) => {
        val executorConfig = {
          ...
          val customCLI = flinkShims
            .getCustomCli(frontend, commandLine)
            .asInstanceOf[CustomCommandLine]
          ...
          val flinkDistJar = new File(s"$flinkLocalHome/lib")
            .list()
            .filter(_.matches("flink-dist_.*\\.jar")) match {
            case Array() =>
              throw new IllegalArgumentException(
                s"[StreamX] can no found flink-dist jar in $flinkLocalHome/lib"
              )
            case array if array.length == 1 =>
              s"$flinkLocalHome/lib/${array.head}"
            case more =>
              throw new IllegalArgumentException(
                s"[StreamX] found multiple flink-dist jar in $flinkLocalHome/lib,[${more.mkString(",")}]"
              )
          }
        ...
        val clusterClient = try clusterDescriptor
    ...
    val effectiveConfig = clusterClient match {
      ...
      case None =>
        getEffectiveConfiguration(config, clusterConfig, null, flinkShims)
    }
    (effectiveConfig, clusterClient)
  }
  ...
  def parseArgList(
    ...
    val args = target match {
      case YarnDeploymentTarget.SESSION | YarnDeploymentTarget.PER_JOB =>
        ArrayBuffer[String]("-m", "yarn-cluster")
      case _ => ArrayBuffer[String]()
    }
    config.yarnConfig match {
      case Some(yarnConfig) =>
        yarnConfig.jobManagerMemory.foreach(
          jmMem => args ++= Seq("-yjm", jmMem)
        )
        yarnConfig.taskManagerMemory.foreach(
          tmMem => args ++= Seq("-ytm", tmMem)
        )
  ...
  private[this] def createRemoteConfig(
      config: Config,
      flinkConfig: Configuration
  ...
  private[this] def createLocalCluster(
      flinkConfig: Configuration
  ): (Configuration, Option[MiniClusterClient]) = {
  ...
  private[this] def getEffectiveConfiguration(
      config: Config,
      flinkConfig: Configuration,
      target: YarnDeploymentTarget,
      flinkShims: FlinkShims
