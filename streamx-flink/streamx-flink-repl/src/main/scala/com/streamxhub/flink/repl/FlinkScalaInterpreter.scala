...
import com.streamxhub.flink.repl.FlinkShell._
import com.streamxhub.flink.repl.util.DependencyUtils
...
class FlinkScalaInterpreter(val properties: Properties) {
  ...
  private def initFlinkConfig(): Config = {
    ...
    mode = ExecutionMode.withName(
      properties.getProperty("flink.execution.mode", "yarn").toUpperCase)
    ...
    this.defaultSqlParallelism = 1 //configuration.getInteger(ExecutionConfigOptions.TABLE_EXEC_RESOURCE_DEFAULT_PARALLELISM)
