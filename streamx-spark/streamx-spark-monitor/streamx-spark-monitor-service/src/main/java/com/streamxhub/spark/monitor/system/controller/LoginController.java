...
import com.streamxhub.spark.monitor.common.annotation.Limit;
import com.streamxhub.spark.monitor.common.domain.ActiveUser;
...
import com.streamxhub.spark.monitor.common.utils.*;
import com.streamxhub.spark.monitor.system.authentication.JWTToken;
...
public class LoginController {
    ...
    public RestResponse login(
        ...
        if (user == null) {
            throw new StreamXException(errorMessage);
        }
        if (!StringUtils.equals(user.getPassword(), password)) {
            throw new StreamXException(errorMessage);
        }
        if (User.STATUS_LOCK.equals(user.getStatus())) {
            throw new StreamXException("账号已被锁定,请联系管理员！");
        }
        ...
        String token = StreamXUtil.encryptToken(JWTUtil.sign(username, password));
