...
/**
  ...
  *
  */
class RedisSink[T <: scala.Product: ClassTag: TypeTag](
  ...
  private lazy val port =
    prop.getProperty("port", Protocol.DEFAULT_PORT.toString).toInt
  private lazy val auth = prop.getProperty("auth", null)
  private lazy val db =
    prop.getProperty("db", Protocol.DEFAULT_DATABASE.toString).toInt
  private lazy val timeout =
    prop.getProperty("timeout", Protocol.DEFAULT_TIMEOUT.toString).toInt
...
object RedisSink {
  ...
  def apply[T <: scala.Product: ClassTag: TypeTag](rdd: RDD[T]) =
    new RedisSink[T](rdd.sparkContext)
}
