...
private[kafka] class HBaseOffset(val sparkConf: SparkConf) extends Offset {
  ...
  private lazy val table: Table = {
    ...
    if (!conn.getAdmin.tableExists(TableName.valueOf(tableName))) {
      val tableDesc: HTableDescriptor = new HTableDescriptor(
  ...
  override def get(
    ...
    logInfo(
      s"storeType:HBase,getOffsets [$groupId,${offsetMaps.mkString(",")}] "
    )
  ...
  override def update(
    ...
    table.put(puts)
    logInfo(
      s"storeType:HBase,updateOffsets [ $groupId,${offsetInfos.mkString(",")} ]"
    )
  }
  ...
  override def delete(groupId: String, topics: Set[String]): Unit = {
    ...
    logInfo(s"storeType:HBase,deleteOffsets [ $groupId,${topics
      .mkString(",")} ] ${deletes.mkString(" ")}")
  }
}
