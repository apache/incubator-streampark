...
class SingleTrackingTaskTest {
  ...
  val trkMonitor: DefaultFlinkTrkMonitor = FlinkTrkMonitorFactory.createInstance().asInstanceOf[DefaultFlinkTrkMonitor]
  ...
  @Test def testSingleTrackingFlinkMetrics(): Unit = {
    // session
    var metrics = trkMonitor.metricsWatcher.collectMetrics(SESSION, "flink-session", "default")
    println(s"result1 = ${metrics.map(_.toString).getOrElse("empty result")}")
    // application
    metrics = trkMonitor.metricsWatcher.collectMetrics(APPLICATION, "flink-app2", "default")
    println(s"result2 = ${metrics.map(_.toString).getOrElse("empty result")}")
    metrics = trkMonitor.metricsWatcher.collectMetrics(APPLICATION, "flink-app3", "default")
