      ...
      <a-list>
        <a-list-item v-for="(item,index) in settings" :key="index">
          <a-list-item-meta style="width: 50%">
            ...
            <svg-icon class="avatar" name="emailfrom" size="large" slot="avatar" v-if="item.key === 'alert.email.from'"></svg-icon>
            <svg-icon class="avatar" name="mail" size="large" slot="avatar" v-if="item.key === 'alert.email.sender'"></svg-icon>
