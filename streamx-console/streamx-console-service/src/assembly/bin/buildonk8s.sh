...
configureK8sConfigfile() {
  read -p 'Please input K8S cluster server:' server
  read -p 'Please input K8S user token:' token
  sed -i '' "s!server:.*!server: $server!" ../plugins/config
  sed -i '' "s!token:.*!token: $token\!" ../plugins/config
}
...
main() {
  ...
  docker buildx build --push --platform linux/amd64 --build-arg DB=$DB -f ../plugins/Dockerfile -t $imageName ../
  if [ $? -eq 0 ]; then
