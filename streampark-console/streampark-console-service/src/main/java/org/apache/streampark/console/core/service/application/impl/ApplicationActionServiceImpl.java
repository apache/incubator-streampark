...
public class ApplicationActionServiceImpl extends ServiceImpl<ApplicationMapper, Application>
    ...
    @Autowired
    private FlinkK8sDataTypeConverterStub flinkK8sDataTypeConverter;
    ...
    public void start(Application appParam, boolean auto) throws Exception {
        ...
        if (FlinkExecutionMode.YARN_APPLICATION == application.getFlinkExecutionMode() || FlinkExecutionMode.YARN_PER_JOB == application.getFlinkExecutionMode() || FlinkExecutionMode.YARN_SESSION == application.getFlinkExecutionMode()) {
            String jobName = appParam.getJobName();
            if (checkAppRepeatInYarn(jobName)) {
                ApiAlertException.throwIfTrue(
                    !application.isCanBeStart(), "[StreamPark] The same task name is already running in the yarn queue");
            }
        }
    ...
    private boolean checkAppRepeatInYarn(String jobName) {
        ...
        try {
            return yarnClient.getApplications().stream().anyMatch(
                applicationReport -> applicationReport.getName().equals(jobName));
