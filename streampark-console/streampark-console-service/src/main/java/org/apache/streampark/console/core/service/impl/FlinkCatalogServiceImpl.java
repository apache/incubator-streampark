...
public class FlinkCatalogServiceImpl extends ServiceImpl<FlinkCatalogMapper, FlinkCatalog>
    implements FlinkCatalogService {
  @Override
  ...
  public boolean existsByCatalogName(String name) {
    return getBaseMapper()
        .exists(new LambdaQueryWrapper<FlinkCatalog>().eq(FlinkCatalog::getCatalogName, name));
  }

  @Override
  public void create(FlinkCatalog flinkCatalog) {
    existsByCatalogName(flinkCatalog.getCatalogName());
    this.save(flinkCatalog);
  }

  @Override
  public void update(FlinkCatalog flinkCatalog) {
    existsByCatalogName(flinkCatalog.getCatalogName());
    this.save(flinkCatalog);
  }
}
