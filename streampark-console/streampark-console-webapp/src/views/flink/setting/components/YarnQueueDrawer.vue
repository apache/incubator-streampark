...
<script lang="ts">
  ...
  import { renderYarnQueueLabel } from '/@/views/flink/setting/hooks/useYarnQueueRender';
            ...
            rules: [
              {
                validator: async (_, value) => {
                  const res = await fetchCheckYarnQueue({ queueLabel: value, teamId: getUserTeamId() });
                  if (res.status === 0) {
                    return Promise.resolve();
                  } else {
                    switch (res.status){
                      case 1:
                        return Promise.reject(t('flink.setting.yarnQueue.checkResult.existedHint'));
                      case 2:
                        return Promise.reject(t('flink.setting.yarnQueue.checkResult.invalidFormatHint'));
                      case 3:
                        return Promise.reject(t('flink.setting.yarnQueue.checkResult.emptyHint'));
                    }
                  }
                },
                trigger: 'blur',
