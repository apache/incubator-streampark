...
export default {
  ...
  methods: {
    ...
    handleCheckRebuild(values) {
      if (this.project.url !== values.url || this.project.branches !== values.branches
        || this.project.pom !== values.pom) {
        this.buildState = -2
      } else {
        this.buildState = this.project.buildState
      }
    },
              ...
              if (this.branchList.indexOf(values.branches) === -1) {
                ...
                update({
                  ...
                  if (updated) {
                    ...
                    this.$swal.fire(
                      'Failed',
                      'Project update failed ..>﹏<.. <br><br>' + resp['message'],
                      'error'
                    )
                  }
                }).catch((error) => {
