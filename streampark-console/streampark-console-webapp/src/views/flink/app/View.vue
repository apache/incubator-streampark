  ...
  methods: {
      ...
      this.$swal.fire({
        ...
        build({
          ...
          if (!resp.data) {
            this.$swal.fire(
              'Failed',
              'lanuch application failed, ' + resp.message.replaceAll(/\[StreamPark]/g, ''),
              'error'
            )
          }
        })
            ...
            start({
              ...
              if (!resp.data) {
                this.$swal.fire({
                  ...
                  html: '<pre class="propException"> startup failed, ' + resp.message.replaceAll(/\[StreamPark]/g, '') + '</pre>',
      ...
      if (this.flameGraph) {
        weburl({}).then((resp) => {
          if (resp.data == null || resp.data === '') {
            this.$swal.fire(
              'Failed',
              ' flameGraph enable Failed <br><br> StreamPark Webapp address not defined <br><br> please check!',
              'error'
            )
            this.flameGraph = false
          }
    ...
    handleEdit(app) {
      ...
      if (app.appType === 1) {
        ...
        this.$router.push({'path': '/flink/app/edit_streampark'})
