...
<script setup lang="ts">
  ...
  import BuildLayer from '/@/views/flink/app/components/AppView/BuildLayer.vue';
  import { useTimeoutFn } from '@vueuse/core';
  import { fetchBuildProgressDetail } from '/@/api/spark/build';
  import { useI18n } from '/@/hooks/web/useI18n';
  import { SparkApplication } from '/@/api/spark/app.type';
  defineOptions({
    name: 'SparkBuildDrawer',
  });
  ...
  async function handleFetchBuildDetail() {
    try {
      ...
      const res = await fetchBuildProgressDetail(+unref(appId)!);
  ...
  const [registerBuild] = useDrawerInner((data: { app: SparkApplication }) => {
...
<template>
  <BasicDrawer
    ...
    :title="t('spark.app.view.buildTitle')"
    width="500"
    :closable="true"
    @close="stop()"
  >
    ...
    <template v-if="appBuildDetail.pipeline != null && appBuildDetail.pipeline.hasError">
      ...
      <BasicDrawer
        ...
        :title="t('spark.app.view.errorLog')"
        width="800"
        :closable="true"
      >
        <div>
          <h3>{{ t('spark.app.view.errorSummary') }}</h3>
          <br />
          <p>{{ appBuildDetail.pipeline.errorSummary }}</p>
          <Divider />
          <h3>{{ t('spark.app.view.errorStack') }}</h3>
          <br />
