  ...
  export default defineComponent({
    ...
    setup() {
        ...
        return [
          {
            ...
            tooltip: t('system.user.operation.modify'),
            auth: 'user:update',
          ...
          },
          {
            ...
            tooltip: t('system.user.operation.viewUserDetailInfoMessage'),
            onClick: handleView.bind(null, record),
          },
          {
            icon: 'bx:reset',
            auth: 'user:reset',
            tooltip: t('system.user.operation.resetUserPasswordMessage'),
            ifShow: () => record.username !== 'admin' || userName.value === 'admin',
            popConfirm: {
              title: t('system.user.operation.resetUserPasswordAlertMessage'),
          ...
          },
          {
            ...
            tooltip: t('system.user.operation.deleteUser'),
            popConfirm: {
              title: t('system.user.operation.deleteAlertMessage'),
      ...
      async function handleDelete(record: UserListRecord) {
        const hide = createMessage.loading(t('system.user.operation.deleteUserProcessMessage'));
        try {
          await deleteUser({ userId: record.userId });
          createMessage.success(t('system.user.operation.deleteUserSuccessful'));
          reload();
        } catch (error) {
      ...
      async function handleReset(record: Recordable) {
        const hide = createMessage.loading(t('system.user.operation.resetUserPasswordProcessMessage'));
        try {
          await resetPassword({ usernames: record.username });
          Swal.fire(
          t('system.user.operation.resetPasswordSuccessMessagePrefix') +
              record.username +
            t('system.user.operation.resetPasswordSuccessMessageSuffix'),
            '',
            'success',
          );
        } catch (error) {
      ...
      function handleSuccess() {
        createMessage.success(t('system.user.operation.createUserSuccessfulMessage'));
        reload();
      }
