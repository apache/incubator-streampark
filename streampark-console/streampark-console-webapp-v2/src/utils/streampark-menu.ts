...
export function streamParkMenuToRowRoutes(
  ...
  for (const route of routes) {
    ...
    const activeMenu = hidden && activeMenuPath
      ? activeMenuPath
    ...
    if (isLayout) {
      ...
      if (route.children?.length)
        rows.push(...streamParkMenuToRowRoutes(route.children as AppRouteRecordRaw[], currentId, activeMenuPath))
    }
    else {
      rows.push({
        id: currentId,
      ...
      if (route.children?.length)
        rows.push(...streamParkMenuToRowRoutes(route.children as AppRouteRecordRaw[], currentId, childActiveMenu))
    }
  }
...
export function normalizeStreamParkMenu(raw: AppRouteRecordRaw[]): AppRoute.RowRoute[] {
  ...
  let tree = raw
  if (raw.length === 1 && raw[0].children?.length)
    tree = raw[0].children as AppRouteRecordRaw[]
