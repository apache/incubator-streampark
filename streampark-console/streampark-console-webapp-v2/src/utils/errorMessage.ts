...
export function formatApiErrorMessage(raw: unknown): string {
    if (raw == null) return ''
    if (typeof raw === 'string') return raw.replace(STREAMPARK_PREFIX, '').trim()
    if (typeof raw === 'object') {
        const record = raw as Recordable
        const nested = record.message ?? record.msg ?? record.error ?? record.errorMessage
        if (nested != null && nested !== raw) return formatApiErrorMessage(nested)
    }
...
export function resolveHttpStatusMessage(status: number): string {
    const resolver = ERROR_STATUS[status as keyof typeof ERROR_STATUS] ?? ERROR_STATUS.default
    return resolver()
}
...
export function throwApiFailure(
    result: Pick<
        ...
        'isSuccess' | 'message' | 'code' | 'errorNotified'
    >,
    fallback?: string,
): never {
    throw new ApiRequestError(resolveResultMessage(result, fallback), Boolean(result.errorNotified))
}
...
export function showResultError(
    ...
    if (result.isSuccess || result.errorNotified) return
...
export function showCatchError(error: unknown, fallback?: string) {
    if (error instanceof ApiRequestError && error.errorNotified) return
    const message =
        error instanceof Error ? formatApiErrorMessage(error.message) : formatApiErrorMessage(error)
