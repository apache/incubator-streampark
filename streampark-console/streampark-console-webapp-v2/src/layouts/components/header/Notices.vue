...
const activeList = computed(() =>
    currentTab.value === 1 ? exceptionList.value : messageList.value,
)
...
const wsUrl = computed(() => {
    ...
    if (!userStore.getToken || !userId) return ''
...
async function loadNotify(type: number) {
    ...
    try {
        ...
        if (type === 1) exceptionList.value = records
        else messageList.value = records
    } finally {
        loading.value = false
    }
}
...
function prependNotify(type: number, item: NotifyItem) {
    if (type === 1) exceptionList.value = [item, ...exceptionList.value]
    else messageList.value = [item, ...messageList.value]
}
...
watch(currentTab, (type) => {
    if (type === 1 && !exceptionList.value.length) loadNotify(1)
    if (type === 2 && !messageList.value.length) loadNotify(2)
})
                ...
                <n-tab-pane :name="1">
                    <template #tab>
                        <n-space class="w-180px" justify="center">
                            ...
                            <n-badge
                                type="error"
                                :value="exceptionList.filter((i) => i.readed === 0).length"
                                :max="99"
                            />
                        </n-space>
                    </template>
                            ...
                            >
                                <n-thing content-indented>
                                    ...
                                    <template #avatar>
                                        <n-icon
                                            size="24"
                                            :color="item.type === 1 ? '#d03050' : '#2080f0'"
                                        >
                                            <IonIcon
                                                :name="
                                                    item.type === 1
                ...
                <n-tab-pane :name="2">
                    <template #tab>
                        <n-space class="w-180px" justify="center">
                            ...
                            <n-badge
                                type="info"
                                :value="messageList.filter((i) => i.readed === 0).length"
                                :max="99"
                            />
                        </n-space>
                    </template>
