...
function bindVerticalMenu(
  ...
  return {
    ...
    verticalMenuProps: {
      ...
      onUpdateValue: (key) => { activeKey.value = key },
      onUpdateExpandedKeys: (keys) => { expandedKeys.value = keys },
    },
  }
}
  ...
  const layout = computed<LayoutMenuReturn>(() => {
    ...
    switch (mode.value) {
      ...
      case 'two-column': {
        ...
        const info = verticalMenuActiveKey != null
          ? menuKeyToMetaMap.value.get(verticalMenuActiveKey as string | number)
          : undefined
        const verticalExtraMenuData = (info?.item[childrenField as keyof MenuOption] as MenuOption[] | undefined) ?? []
        return {
          horizontalMenuProps: {},
          verticalMenuProps: {
            mode: 'vertical',
      ...
      case 'mixed-sidebar': {
        ...
        const info = horizontalMenuActiveKey != null
          ? menuKeyToMetaMap.value.get(horizontalMenuActiveKey as string | number)
          : undefined
        const verticalMenuData = (info?.item[childrenField as keyof MenuOption] as MenuOption[] | undefined) ?? []
        return {
          horizontalMenuProps: {
            mode: 'horizontal',
            responsive: true,
      ...
      case 'mixed-two-column': {
        ...
        const horizontalMenuData = splitMenuData(m, 1, { childrenField })[0] ?? []
        const topInfo = horizontalMenuActiveKey != null
          ? menuKeyToMetaMap.value.get(horizontalMenuActiveKey as string | number)
          : undefined
        const childData = (topInfo?.item[childrenField as keyof MenuOption] as MenuOption[] | undefined) ?? []
        const verticalMenuData = splitMenuData(childData, 1, { childrenField })[0] ?? []
        const extraInfo = verticalMenuActiveKey != null
          ? menuKeyToMetaMap.value.get(verticalMenuActiveKey as string | number)
          : undefined
        const verticalExtraMenuData = (extraInfo?.item[childrenField as keyof MenuOption] as MenuOption[] | undefined) ?? []
        return {
          horizontalMenuProps: {
            mode: 'horizontal',
            responsive: true,
      ...
      case 'full-content':
        return { horizontalMenuProps: {}, verticalMenuProps: {}, verticalExtraMenuProps: {} }
      case 'sidebar':
      case 'vertical':
      default:
