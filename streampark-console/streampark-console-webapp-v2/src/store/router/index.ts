...
export const useRouteStore = defineStore('route-store', {
    ...
    actions: {
        ...
        async initRouteInfo() {
            ...
            if (permissions?.length) permissionStore.setPermCodeList(permissions)

            const result = await fetchMenuRouter()
            if (!result.isSuccess || !result.data) throw new Error('Failed to load menu routes')
        ...
        async initAuthRoute() {
            ...
            try {
                ...
                if (!rowRoutes?.length) throw new Error('Empty route list')
