...
const metaFields: AppRoute.MetaKeys[]
  = ['title', 'icon', 'requiresAuth', 'roles', 'keepAlive', 'hide', 'order', 'href', 'activeMenu', 'withoutTab', 'pinTab', 'menuType', 'componentName']
...
function normalizeComponentRel(componentPath: string): string {
  ...
  if (COMPONENT_ALIASES[rel])
    rel = COMPONENT_ALIASES[rel]
...
function resolveViewComponent(componentPath: string) {
  const normalized = componentPath.startsWith('/src')
    ? componentPath
    : `/src${componentPath}`

  const nativePath = resolveNativeViewPath(normalized)
  if (nativeViewModules[nativePath])
    return nativeViewModules[nativePath]
...
function flattenAuthRoutes(routes: AppRoute.Route[]): AppRoute.Route[] {
  ...
  const visit = (items: AppRoute.Route[]) => {
    for (const item of items) {
      if (item.component)
        pages.push({ ...item, children: undefined })
...
function buildDirectoryRedirectRoutes(rowRoutes: AppRoute.RowRoute[]): AppRoute.Route[] {
  ...
  const findFirstVisiblePage = (parentId: number): AppRoute.RowRoute | undefined => {
    ...
    for (const child of children) {
      if (child.menuType === 'page' && child.componentPath)
        return child
  ...
  return userRoutes
    ...
    .map((item) => {
      ...
      const target: MenuOption = {
        ...
        key: item.path,
        icon: item.meta.menuType === 'dir' && item.meta.icon ? renderIcon(item.meta.icon) : undefined,
      }
      return target
    })
}
