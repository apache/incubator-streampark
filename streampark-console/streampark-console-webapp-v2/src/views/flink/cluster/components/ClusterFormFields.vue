...
<template>
  <n-form ref="formRef" :model="model" :rules="rules" label-placement="top">
    ...
    <template v-if="isK8sSession">
      <n-form-item :label="t('setting.flinkCluster.form.k8sClusterId')">
        <n-auto-complete
          ...
          :options="historyRecord.k8sSessionClusterId.map(v => ({ label: v, value: v }))"
          placeholder="default"
          clearable
        />
      </n-form-item>
    ...
    <template v-if="isSessionMode">
      ...
      <n-form-item v-for="key in model.totalOptions" :key="`total-${key}`" :label="memoryLabel(key, totalMemoryOptions)">
      ...
      <n-form-item v-for="key in model.jmOptions" :key="`jm-${key}`" :label="memoryLabel(key, jmMemoryOptions)">
      ...
      <n-form-item v-for="key in model.tmOptions" :key="`tm-${key}`" :label="memoryLabel(key, tmMemoryOptions)">
        ...
        >
          <template #default="{ value }">
            <div class="flex w-full gap-8px">
              ...
              <n-input v-model:value="value.value" placeholder="value" class="flex-1" />
            </div>
          </template>
        </n-dynamic-input>
      </n-form-item>
