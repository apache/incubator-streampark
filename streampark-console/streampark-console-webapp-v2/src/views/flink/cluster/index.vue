...
async function handleStart(row: FlinkCluster) {
    const msg = window.$message?.loading(
    ...
    try {
        ...
        window.$message?.success(
            t('setting.flinkCluster.operateMessage.flinkClusterHasStartedSuccessful'),
        )
        loadData(true)
    } catch (e: any) {
...
async function handleShutdown(row: FlinkCluster) {
    const msg = window.$message?.loading(
        t('setting.flinkCluster.operateMessage.flinkClusterIsCanceling'),
        { duration: 0 },
    )
    try {
        ...
        <div class="mb-16px flex flex-wrap items-center justify-between gap-12px">
            ...
            <n-space>
                ...
                <n-button
                    ...
                    type="primary"
                    @click="router.push('/flink/add_cluster')"
                >
                    {{ t('common.add') }}
                </n-button>
            </n-space>
