...
export function useClusterForm() {
                    ...
                    if (value === DeployMode.YARN_SESSION) {
                        ...
                        if (!result.isSuccess || !result.data)
                            return Promise.reject(
                                t(
                                    'setting.flinkCluster.operateMessage.hadoopEnvInitializationFailed',
                                ),
                            )
                    }
    ...
    async function loadReferenceData() {
        const [envResult, alertResult, nsResult, clusterIdsResult, imagesResult] =
            await Promise.all([
                fetchListFlinkEnv(),
                fetchAlertSetting(),
                fetchK8sNamespaces(),
        ...
        if (envResult.isSuccess) flinkEnvs.value = envResult.data ?? []
        if (alertResult.isSuccess) alerts.value = alertResult.data ?? []
        if (nsResult.isSuccess) historyRecord.k8sNamespace = nsResult.data ?? []
        if (clusterIdsResult.isSuccess)
            historyRecord.k8sSessionClusterId = clusterIdsResult.data ?? []
        if (imagesResult.isSuccess) historyRecord.flinkImage = imagesResult.data ?? []
    }
