...
function updateTitleLens(records: AppListRecord[]) {
  ...
  for (const cur of records) {
    ...
    if (optionState === OptionStateEnum.NONE) {
      ...
      if (stateStr && stateStr.length > maxState.length)
        maxState = stateStr
    }
    else {
    ...
    if (releaseStr && releaseStr.length > maxRelease.length)
      maxRelease = releaseStr
    const buildStr = buildStatusMap[buildStatus]?.title
    if (buildStr && buildStr.length > maxBuild.length)
      maxBuild = buildStr
  }
...
function cleanupOptionApps(records: AppListRecord[]) {
  ...
  for (const x of records) {
    if (x.optionState === OptionStateEnum.NONE) {
      ...
      if (optionApps.savepointing.get(x.id) && timestamp - optionApps.savepointing.get(x.id)! > 2000)
...
function handleOptionApp(data: { type: 'starting' | 'stopping' | 'release' | 'savepointing', key: string, value: number }) {
...
async function handleRelease(app: AppListRecord, force = false) {
  ...
  try {
    ...
    if (ok) {
      ...
      window.$message?.error((message || t('flink.app.release.releaseFail')).replaceAll(/\[StreamPark]/g, ''))
    }
  }
  catch (e: any) {
    showCatchError(e, t('sys.api.apiRequestFailed'))
...
async function openStartModal(app: AppListRecord) {
  ...
  const records = result.isSuccess
    ? resolveListData<SavepointHistoryItem>(result.data as { records?: SavepointHistoryItem[], total?: number | string }).records
    : []
...
async function submitCopy() {
  ...
  if (!(await validateCopyJobName(jobName)))
    return
  try {
    ...
    if (status === 'success') {
      ...
      window.$message?.error(String(payload?.message || t('flink.app.operation.copyFail')).replaceAll(/\[StreamPark]/g, ''))
    }
  }
  catch (e: any) {
...
function actionBtn(icon: string, tip: string, onClick?: () => void, cls?: string, disabled = false) {
...
function buildRowActions(row: AppListRecord) {
  ...
  if (hasPermission('app:update')) {
    actions.push(actionBtn(I.edit, t('flink.app.operation.edit'), () => goEdit(row), 'e2e-flinkapp-edit-btn'))
  }
  if (hasPermission('app:release') && [
    ReleaseStateEnum.FAILED,
    ReleaseStateEnum.NEED_RELEASE,
    ReleaseStateEnum.NEED_ROLLBACK,
  ].includes(row.release) && row.optionState === OptionStateEnum.NONE) {
    actions.push(actionBtn(I.release, t('flink.app.operation.release'), () => handleRelease(row), 'e2e-flinkapp-release-btn'))
  }
  if (hasPermission('app:release') && (
    [ReleaseStateEnum.FAILED, ReleaseStateEnum.RELEASING].includes(row.release)
    || row.optionState === OptionStateEnum.RELEASING
  )) {
    actions.push(actionBtn(I.releaseDetail, t('flink.app.operation.releaseDetail'), () => {
  ...
  if (hasPermission('app:start') && handleIsStart(row, optionApps)) {
    actions.push(actionBtn(I.start, t('flink.app.operation.start'), () => handleStartCheck(row), 'e2e-flinkapp-startup-btn'))
  }
  if (hasPermission('app:cancel') && row.state === AppStateEnum.RUNNING && row.optionState === OptionStateEnum.NONE) {
    actions.push(actionBtn(I.pause, t('flink.app.operation.cancel'), () => openStopModal(row), 'e2e-flinkapp-cancel-btn'))
  }
  if (
    ...
    actions.push(actionBtn(I.savepoint, t('flink.app.operation.savepoint'), () => openSavepointModal(row), 'e2e-flinkapp-savepoint-btn'))
  }
  if (hasPermission('app:detail')) {
    actions.push(actionBtn(I.detail, t('flink.app.operation.detail'), () => goDetail(row), 'e2e-flinkapp-detail-btn'))
  }

  const dropdownOptions: DropdownOption[] = []
  if (hasPermission('app:detail') && [DeployMode.KUBERNETES_SESSION, DeployMode.KUBERNETES_APPLICATION].includes(row.deployMode)) {
  ...
  if (hasPermission('app:mapping') && [
    AppStateEnum.ADDED, AppStateEnum.FAILED, AppStateEnum.CANCELED, AppStateEnum.KILLED,
    AppStateEnum.SUCCEEDED, AppStateEnum.TERMINATED, AppStateEnum.POS_TERMINATED,
    AppStateEnum.FINISHED, AppStateEnum.SUSPENDED, AppStateEnum.LOST,
  ].includes(row.state)) {
    dropdownOptions.push({
  ...
  if (dropdownOptions.length) {
    actions.push(
      h(NDropdown, {
        ...
        default: () => h(NButton, { quaternary: true, size: 'small', class: 'e2e-flinkapp-more-btn' }, {
          default: () => '...',
        }),
      }),
    )
...
function handleColumnWidthChange(width: number, _limitedWidth: number, column: { key?: string | number }) {
  if (column.key === 'jobName')
    jobNameColumnWidth.value = width
  if (column.key === 'release')
    releaseColumnWidth.value = width
}
...
const columns = computed<DataTableColumns<AppListRecord>>(() => [
  ...
  },
  {
    ...
    key: 'jobType',
    width: 90,
    render: row => (row.jobType === JobTypeEnum.JAR ? 'JAR' : row.jobType === JobTypeEnum.SQL ? 'SQL' : 'PY'),
  },
  {
  ...
  },
  {
    ...
    key: 'state',
    width: 130,
    render: row => h(AppStateTag, { option: 'state', data: row, maxTitle: titleLenRef.maxState }),
  },
  {
    ...
    minWidth: 130,
    resizable: true,
    render: row => h(AppStateTag, { option: 'release', data: row, maxTitle: titleLenRef.maxRelease }),
  },
  {
    title: t('flink.app.buildColumn'),
    key: 'buildStatus',
    width: 110,
    render: row => h(AppStateTag, { option: 'build', data: row, maxTitle: titleLenRef.maxBuild }),
  },
  {
...
async function initTagsOptions() {
  ...
  for (const record of records) {
    if (record.tags) {
      String(record.tags).split(',').filter(Boolean).forEach(tag => tags.add(tag))
    }
  }
...
function hasActiveOperations() {
  return optionApps.starting.size > 0
...
<template>
  <div class="h-full flex flex-col">
          ...
          <n-grid :x-gap="12" :cols="24" responsive="screen" item-responsive>
            ...
            <n-form-item-gi :span="5" :xs="24" :sm="12" :md="5">
              <n-select
                ...
                :options="users.map(u => ({ label: u.nickName || u.username, value: u.userId }))"
                @update:value="handleSearch"
              />
            </n-form-item-gi>
          </n-grid>
        </n-form>
        <n-button id="e2e-flinkapp-create-btn" v-auth="'app:create'" type="primary" @click="router.push('/flink/app/add')">
          {{ t('common.add') }}
        </n-button>
      </div>
    ...
    <n-modal v-model:show="copyModalVisible" preset="card" :style="{ width: '480px' }" :title="t('flink.app.operation.copyModalTitle')">
      <n-form label-placement="top">
        <n-form-item :label="t('flink.app.operation.copyJobNameLabel')" required>
          <n-input v-model:value="copyJobName" :placeholder="t('flink.app.operation.copyJobNamePlaceholder')" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
    ...
    <n-modal v-model:show="mappingModalVisible" preset="card" :style="{ width: '520px' }" :title="t('flink.app.operation.mappingModalTitle')">
      <n-form label-placement="top">
        ...
        <n-form-item
          v-if="mappingApp && [DeployMode.YARN_PER_JOB, DeployMode.YARN_SESSION, DeployMode.YARN_APPLICATION].includes(mappingApp.deployMode)"
          :label="t('flink.app.operation.mappingYarnAppIdLabel')"
        >
          <n-input v-model:value="mappingForm.clusterId" :placeholder="t('flink.app.operation.mappingYarnAppIdPlaceholder')" />
        </n-form-item>
        <n-form-item :label="t('flink.app.operation.mappingJobIdLabel')" required>
          <n-input v-model:value="mappingForm.jobId" :placeholder="t('flink.app.operation.mappingJobIdPlaceholder')" />
        </n-form-item>
      </n-form>
      <template #footer>
        <n-space justify="end">
          ...
          <n-button type="primary" class="e2e-flinkapp-remapping-btn" @click="submitMapping">
            {{ t('common.apply') }}
          </n-button>
        </n-space>
      </template>
