...
export function useEdit() {
  ...
  async function handleGetApplication() {
    ...
    if (!result.isSuccess)
      throwApiFailure(result, 'Failed to load application')
        ...
        if (
          ...
          if (k.startsWith('jobmanager.memory.')) {
            ...
            if (k === 'jobmanager.memory.jvm-overhead.fraction')
              ...
              fieldValueOptions.jmOptionsItem[key] = Number.parseInt(v)
          }
            ...
            if (
              ...
              fieldValueOptions.tmOptionsItem[key] = Number.parseInt(v)
          }
        }
      }
      else {
        if (k === 'taskmanager.numberOfTaskSlots')
          slot = Number.parseInt(v)
        if (k === 'parallelism.default')
          parallelism = Number.parseInt(v)
      }
    }
