...
import { AppListRecord } from '@/types/api/flink/app.type';
import {
  AppStateEnum,
  ConfigTypeEnum,
  DeployMode,
...
export function handleAppBuildStepTimelineColor(step: { status?: number } | null) {
  if (step == null) {
    return 'gray';
  }
  switch (step.status) {
...
export function descriptionFilter(option: Recordable) {
  if (option.unit) {
...
export function handleDependencyJsonToPom(
  json: string | null | undefined,
  pomMap: Map<string, Recordable>,
  jarMap: Map<string, string>,
) {
  if (json != null && json.trim() !== '') {
    const deps = JSON.parse(json) as Recordable;
    const pom = deps.pom as Recordable[] | undefined;
    if (pom && pom.length > 0) {
      pom.forEach((x: Recordable) => {
        ...
        if (exclusions != null && exclusions.length > 0) {
          exclusions.forEach((e: Recordable) => {
...
export const selectLabelFilter = (pattern: string, option: Recordable) =>
  String(option.label ?? '').toLowerCase().includes(pattern.toLowerCase());
