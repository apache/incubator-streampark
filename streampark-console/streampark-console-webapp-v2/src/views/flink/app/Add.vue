...
const flinkEnvs = ref<Array<{ id: string, flinkName: string, isDefault?: boolean, scalaVersion?: string }>>([])
...
const rules: FormRules = {
  jobName: [{ required: true, message: t('flink.app.addAppTips.appNameNotValid'), trigger: 'blur' }],
  versionId: [{ required: true, message: t('flink.app.addAppTips.flinkVersionIsRequiredMessage'), trigger: 'change' }],
}
...
async function handleSubmit() {
  ...
  if (!(await checkJobName()))
    return
  if (!validateResourceFields())
    return
  if (!validateDeployFields())
    return
  ...
  try {
    ...
    if (!ok) {
      const msg = (typeof payload === 'object' ? payload?.message : result.message) || t('sys.api.apiRequestFailed')
