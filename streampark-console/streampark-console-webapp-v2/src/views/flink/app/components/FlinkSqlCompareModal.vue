...
import { fetchFlinkSql } from '@/service'
...
const props = defineProps<{
  ...
  versions: Array<{ label: string, value: string, version?: number, effective?: boolean, candidate?: number }>
}>()
...
async function loadSql(recordId: string, setter: (c: string) => Promise<void>) {
  if (!props.appId)
    return
  const result = await fetchFlinkSql({ id: recordId, appId: props.appId })
  if (!result.isSuccess)
  ...
  >
    <n-spin :show="loading">
      ...
      <n-grid :cols="2" :x-gap="12">
        ...
        <n-gi>
          <n-text depth="3" class="mb-4px block text-12px">
            target v{{ targetOptions.find(o => o.value === targetRecordId)?.version ?? '-' }}
          </n-text>
