...
const { setContent: setPodContent, onUpdateValue: podContentChange } = useMonaco(podTemplateRef, {
    language: 'yaml',
...
async function loadHistoryTemplates(visualType: VisualType) {
    if (historyRecord[visualType].length > 0) return historyRecord[visualType]
    let result
    if (visualType === 'ptVisual') result = await fetchFlinkPodTemplates({})
    else if (visualType === 'jmPtVisual') result = await fetchFlinkJmPodTemplates({})
    else result = await fetchFlinkTmPodTemplates({})
    if (result.isSuccess && result.data) historyRecord[visualType] = result.data
...
async function handleGetInitPodTemplate(visualType: VisualType) {
    ...
    if (result.isSuccess && result.data) handleChoicePodTemplate(visualType, result.data)
}

async function showTemplateHostAliasDrawer(visualType: VisualType) {
    hostAliasVisualType.value = visualType
    let tmplContent = ''
    if (visualType === 'ptVisual') tmplContent = props.podTemplate ?? ''
    else if (visualType === 'jmPtVisual') tmplContent = props.jmPodTemplate ?? ''
    else tmplContent = props.tmPodTemplate ?? ''
    ...
    if (sysResult.isSuccess && sysResult.data) sysHostsAlias.value = sysResult.data

    if (tmplContent) {
        const extractResult = await fetchExtractHostAliasFromPodTemplate({
...
async function handleSelectedTemplateHostAlias() {
    ...
    if (result.isSuccess) hostAliasPreview.value = result.data ?? ''
}
            ...
            >
                <template #header-extra>
                    <n-button
                        text
                        type="primary"
                        @click="handleChoicePodTemplate(historyVisualType, item)"
                    >
