...
async function handleVerifySql(showSuccess = true): Promise<boolean> {
  ...
  try {
    ...
    if (payload?.code === 200 || payload?.data?.verified) {
      if (showSuccess)
        window.$message?.success(t('flink.app.flinkSql.successful'))
      return true
    }
    ...
    >
      ...
      <div v-if="!readonly" class="mb-8px flex shrink-0 flex-wrap justify-end gap-8px">
        ...
        <n-button size="small" @click="toggleFullscreen">
          ...
          {{ fullscreen ? t('flink.app.flinkSql.exit') : t('flink.app.flinkSql.fullScreen') }}
        </n-button>
        <n-button size="small" type="primary" :loading="verifying" @click="handleVerifySql(true)">
