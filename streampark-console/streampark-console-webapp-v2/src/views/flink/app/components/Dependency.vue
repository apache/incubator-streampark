...
async function resolveFlinkEnvs() {
  if (props.flinkEnvs?.length)
    return props.flinkEnvs
  if (cachedFlinkEnvs.value.length)
          ...
          if (exclusion != null) {
            exclusion.split('<exclusion>').forEach((e) => {
              if (e?.length > 0) {
                ...
                if (eGroup && eArtifact) {
                  mvnPom.exclusions!.push({ groupId: eGroup, artifactId: eArtifact })
                }
              }
            })
          }
...
async function handleReloadHistoryUploads() {
  ...
  if (result.isSuccess && result.data)
    historyUploadJars.value = result.data
}
...
function handleHistoryUploadJarsChange(values: string[]) {
  values.forEach((item) => {
    if (!Reflect.has(dependency.jar, item))
...
<template>
  ...
  <div v-if="dependencyRecords.length > 0 || uploadJars.length > 0" class="mt-12px flex flex-col gap-8px">
    <n-alert
    ...
    >
      <template #header>
        <n-space align="center">
          ...
          <n-button text type="error" @click.stop="handleRemovePom(dept)">
            ×
          </n-button>
        </n-space>
      </template>
    ...
    >
      <template #header>
        <n-space align="center">
          <n-tag :color="toTagColor('#108ee9')">
            JAR
          </n-tag>
          {{ jar }}
          <n-button text type="error" @click="handleRemoveJar(jar)">
            ×
          </n-button>
        </n-space>
      </template>
