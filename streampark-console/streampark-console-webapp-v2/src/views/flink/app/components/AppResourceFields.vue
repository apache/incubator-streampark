...
async function loadJars() {
  ...
  if (result.isSuccess)
    jarList.value = result.data ?? []
}

async function loadConfTree() {
  if (!model.value.project || !model.value.module)
    return
  const result = await fetchProjectListConf({ id: model.value.project, module: model.value.module })
  if (result.isSuccess)
    confTree.value = (result.data ?? []) as Recordable[]
}
...
async function handleModuleChange(_module: string | null) {
  ...
  if (model.value.project && model.value.module) {
    await loadConfTree()
    if (model.value.appType === AppTypeEnum.APACHE_FLINK)
      await loadJars()
  }
}
...
async function handleJarChange(jar: string | null) {
  if (!jar || !model.value.project || !model.value.module)
    return
  ...
  if (result.isSuccess && result.data)
    model.value.mainClass = result.data
}
...
async function handleConfigChange(configPath: string | null) {
  ...
  if (nameResult.isSuccess && nameResult.data)
    model.value.jobName = nameResult.data
}
...
function handleUploadJarSelect(resourceName: string | null) {
  ...
  if (resource?.mainClass)
    model.value.mainClass = resource.mainClass
}
...
async function handleCustomJobRequest(data: { file: File }) {
  try {
    ...
    if (result.data?.mainClass)
      model.value.mainClass = result.data.mainClass
  }
  catch (e: any) {
...
watch(
  () => model.value.jobType,
  (jobType) => {
    if (jobType !== JobTypeEnum.SQL)
      model.value.resourceFrom = ResourceFromEnum.PROJECT
  },
)
    ...
    <template v-else-if="isUpload">
      ...
      <n-form-item-gi :span="24" :label="t('flink.app.uploadJobJar')">
        <UploadJobJar :custom-request="handleCustomJobRequest" :loading="uploadLoading">
          <template #uploadInfo>
            <n-alert v-if="model.jar || model.uploadJobJar" type="info" class="mt-8px">
