...
import FlinkSqlEditor from './FlinkSqlEditor.vue'
...
watch(
    ...
    async ([show, recordId, appId]) => {
        ...
        loading.value = true
        try {
            const result = await fetchFlinkSql({ id: recordId, appId })
...
<template>
    <n-modal
        ...
        preset="card"
        :style="{ width: '900px' }"
        :title="`${t('flink.app.detail.detailTab.sqlDetail')} v${version ?? ''}`"
        @update:show="emit('update:show', $event)"
    >
        <n-spin :show="loading">
            <FlinkSqlEditor v-model="sqlContent" readonly height="480px" />
        </n-spin>
    </n-modal>
</template>
