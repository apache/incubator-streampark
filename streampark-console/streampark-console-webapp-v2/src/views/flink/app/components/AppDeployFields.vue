...
async function loadOptions() {
  ...
  if (clusterResult.isSuccess)
    flinkClusters.value = clusterResult.data ?? []
  if (nsResult.isSuccess)
    k8sNamespaces.value = nsResult.data ?? []
  if (sessionResult.isSuccess)
    k8sSessionClusterIds.value = sessionResult.data ?? []
  if (imageResult.isSuccess)
    flinkBaseImages.value = imageResult.data ?? []
}
...
<template>
  ...
  <n-form-item-gi v-if="showYarnQueue" :span="12" :label="t('flink.app.yarnQueue')">
    <n-input
      v-model:value="model.yarnQueue"
      :placeholder="t('flink.app.yarnQueue')"
    />
  </n-form-item-gi>
    <n-form-item-gi v-if="!isK8sDeployMode(model.deployMode)" :span="12" :label="t('flink.app.hadoopUser')">
