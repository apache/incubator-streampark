...
function configActionBtn(icon: string, tip: string, onClick: () => void, cls?: string, disabled = false) {
  return h(NTooltip, { trigger: 'hover' }, {
    trigger: () => h(NButton, { quaternary: true, size: 'small', class: cls, disabled, onClick: disabled ? undefined : onClick }, {
      icon: () => ionIcon(icon),
    }),
    default: () => tip,
  })
...
const configColumns = computed<DataTableColumns<Recordable>>(() => [
  ...
  { title: t('flink.app.detail.columns.confFormat'), key: 'format', width: 100 },
  {
    ...
    width: 100,
    render(row) {
      return h(NTag, { size: 'small', type: row.effective ? 'success' : 'default' }, {
  ...
  { title: t('flink.app.detail.columns.modifyTime'), key: 'createTime', width: 180 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          configActionBtn(I.view, t('common.detail'), () => openConfigView(row.version)),
          configActionBtn(I.compare, t('flink.app.flinkSql.compare'), () => openCompare(row.version)),
          h(NPopconfirm, { onPositiveClick: () => handleDeleteConf(row.id) }, {
            trigger: () => configActionBtn(I.delete, t('common.delText'), () => {}),
...
const flinkSqlColumns = computed<DataTableColumns<Recordable>>(() => [
  ...
  },
  {
    ...
    render(row) {
      if ([CandidateTypeEnum.NEW, CandidateTypeEnum.HISTORY].includes(row.candidate))
        return h(NTag, { size: 'small', type: 'info' }, { default: () => t('flink.app.detail.candidate') })
      return '-'
    },
  },
  { title: t('flink.app.detail.columns.modifyTime'), key: 'createTime', width: 180 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          configActionBtn(I.view, t('flink.app.detail.detailTab.sqlDetail'), () => openSqlView(row)),
          configActionBtn(I.compare, t('flink.app.detail.compareFlinkSql'), () => openSqlCompare(row), undefined, flinkSqlVersions.value.length <= 1),
          h(NPopconfirm, { onPositiveClick: () => handleDeleteFlinkSql(row) }, {
            trigger: () => configActionBtn(I.delete, t('common.delText'), () => {}, undefined, !!row.effective),
...
const optionLogColumns = computed<DataTableColumns<Recordable>>(() => [
  { title: t('flink.app.detail.optionTime'), key: 'optionTime', width: 180 },
  {
    ...
    render(row) {
      const tags: Record<number, { color: string, label: string }> = {
        [OperationEnum.RELEASE]: { color: '#2080f0', label: t('flink.app.detail.operationNames.release') },
        [OperationEnum.START]: { color: '#18a058', label: t('flink.app.detail.operationNames.start') },
        [OperationEnum.SAVEPOINT]: { color: '#36cfc9', label: t('flink.app.detail.operationNames.savepoint') },
        [OperationEnum.CANCEL]: { color: '#f0a020', label: t('flink.app.detail.operationNames.cancel') },
      }
      const meta = tags[row.optionName as number]
      return meta
        ? h(NTag, { size: 'small', color: toTagColor(meta.color) }, { default: () => meta.label })
  ...
  { title: t('flink.app.detail.optionUser'), key: 'optionUser' },
  {
    ...
    render(row) {
      ...
      }, { default: () => (row.success ? t('flink.app.detail.startStatusText.success') : t('flink.app.detail.startStatusText.failed')) })
    },
  },
  {
    title: t('component.table.operation'),
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          !row.success
            ? configActionBtn(I.view, t('flink.app.detail.detailTab.exception'), () => openExecOption(row))
            : null,
          h(NPopconfirm, {
            onPositiveClick: () => handleDeleteLog(row.id),
          }, {
            trigger: () => h(NButton, { quaternary: true, size: 'small' }, { default: () => t('common.delText') }),
            default: () => t('common.confirmDelete'),
          }),
        ].filter(Boolean),
      })
...
const savepointColumns = computed<DataTableColumns<Recordable>>(() => [
  ...
  { title: t('flink.app.detail.checkPointType'), key: 'checkPointType', width: 120 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          configActionBtn(I.copy, t('flink.app.detail.detailTab.copyPath'), () => handleCopyPath(row.path)),
          h(NPopconfirm, {
...
watch(() => [props.app.id, activeTab.value] as const, () => loadTabData(), { immediate: true })
</script>
...
<template>
  ...
  />
  <ExecOptionModal
    v-model:show="execOptionVisible"
    :content="execOptionContent"
  />
</template>
