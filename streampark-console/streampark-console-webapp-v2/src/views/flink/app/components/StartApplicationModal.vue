  ...
  try {
    ...
    if (ok) {
      ...
      window.$dialog?.warning({
        ...
        onPositiveClick: () => {
          router.push({ path: '/flink/app/detail', query: { appId: props.payload!.application.id } })
        },
      })
    }
  }
...
<template>
  ...
  >
    ...
    <n-form ref="formRef" :model="formModel" label-placement="top">
      ...
      <n-form-item v-if="formModel.restoreSavepoint" :label="t('flink.app.detail.detailTab.detailTabName.savepoint')" required>
        <n-select
        ...
        />
        <n-text depth="3" class="mt-4px text-12px">
          {{ savepointOptions.length > 0 ? t('flink.app.view.savepointSwitch') : t('flink.app.view.savepointInput') }}
        </n-text>
      </n-form-item>
      <n-form-item v-if="formModel.restoreSavepoint" :label="t('flink.app.view.ignoreRestored')">
