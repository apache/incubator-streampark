...
import { fetchAppUpdate, fetchConfHistory, fetchFlinkEnvPage, fetchFlinkSqlList, fetchUpload } from '@/service'
...
import { getAppConfType, handleSubmitParams, handleTeamResource, isK8sDeployMode } from '@/views/flink/app/shared/utils'
...
const showUploadJar = computed(() => !isSqlJob.value && app.resourceFrom === ResourceFromEnum.UPLOAD)
const showProjectConf = computed(() => !isSqlJob.value && app.resourceFrom === ResourceFromEnum.PROJECT)
...
async function loadFlinkSqlHistory() {
  if (!app.id || app.jobType !== JobTypeEnum.SQL)
    return
  const result = await fetchFlinkSqlList({ appId: app.id, pageNum: 1, pageSize: 200 })
  if (result.isSuccess)
    flinkSqlHistory.value = resolveListData<{ id: string, version: number, effective?: boolean }>(
      result.data as { records?: Array<{ id: string, version: number, effective?: boolean }>, total?: number | string },
    ).records
}
...
async function loadConfigVersions() {
  ...
  if (!result.isSuccess)
    return
  configVersions.value = (result.data ?? []) as Array<{ id: string, version?: number, effective?: boolean }>
}
...
async function submitSqlJob() {
  ...
  const params: Recordable = {
    ...
    config,
    format: formModel.value.isSetConfig && config ? getAppConfType(formModel.value.configOverride) : null,
...
async function submitCustomJob() {
  ...
  const format = values.strategy === UseStrategyEnum.USE_EXIST
    ? app.format
  ...
  const params: Recordable = {
    ...
    format,
    configId: values.strategy === UseStrategyEnum.USE_EXIST ? (values.configId ?? app.configId ?? null) : null,
    config,
  }
      ...
      <n-grid :cols="24" :x-gap="16">
        ...
        <template v-if="!isSqlJob">
          <n-form-item-gi :span="12" :label="t('flink.app.mainClass')">
            <n-input
              ...
              :placeholder="isPyflinkJob ? t('flink.app.addAppTips.mainClassPlaceholder') : undefined"
            />
          </n-form-item-gi>
          <n-form-item-gi v-if="showUploadJar" :span="24" :label="t('flink.app.uploadJobJar')">
        ...
        >
          <n-select
            ...
            clearable
            :options="flinkSqlHistory.map(item => ({
        ...
        <n-form-item-gi v-if="showCompareConf" :span="24" :label="t('flink.app.detail.compareConfig')">
          <CompareConfPicker
        ...
        <n-form-item-gi v-else-if="!isSqlJob && showUploadJar" :span="24" :label="t('flink.app.appConf')">
          <n-space>
            <n-switch v-model:value="formModel.isSetConfig">
              <template #checked>ON</template>
              <template #unchecked>OFF</template>
