...
const title = computed(() =>
  props.versionId ? t('common.edit') : t('common.add'),
)
...
async function handleSubmit() {
  ...
  try {
    ...
    const result = props.versionId == null
    ...
    if (!result.data) {
      const message = (result.message || t('sys.api.apiRequestFailed')).replaceAll(/\[StreamPark]/g, '')
      window.$message?.error(message)
      return
    }

    const successMessage = props.versionId == null
      ? formModel.value.flinkName.concat(t('setting.flinkHome.operateMessage.createFlinkHomeSuccessful'))
      : formModel.value.flinkName.concat(t('setting.flinkHome.operateMessage.updateFlinkHomeSuccessful'))
