  ...
  username: [
    ...
    { min: 2, max: 20, message: t('system.user.form.min'), trigger: 'blur' },
    {
      asyncValidator: async (_rule, value: string) => {
        if (!isCreate.value || !value || value.length < 2 || value.length > 20)
          return
        const result = await fetchCheckUserName({ username: value })
        if (!result.isSuccess || !result.data)
          throw new Error(t('system.user.form.exist'))
      },
      trigger: 'blur',
    },
  ],
  nickName: [{ required: isCreate.value, message: t('system.user.form.required'), trigger: 'blur' }],
