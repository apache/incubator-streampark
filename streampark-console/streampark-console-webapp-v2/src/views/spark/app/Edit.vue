...
async function loadApp() {
    ...
    if (!result.isSuccess || !result.data) throwApiFailure(result, t('sys.api.apiRequestFailed'))
...
async function handleSubmit(values: Recordable) {
    ...
    try {
        ...
        if (params.config) params.config = encryptByBase64(params.config)
        else params.config = null

        if (values.jobType === JobTypeEnum.SQL) params.sqlId = sqlId.value
