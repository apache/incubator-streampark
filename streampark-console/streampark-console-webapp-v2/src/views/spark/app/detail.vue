...
const appNotRunning = computed(() =>
  app.state !== AppStateEnum.RUNNING || yarnUrl.value === null,
)
...
function renderOperationTag(name?: string | number) {
  ...
  return h(NTag, { size: 'small', color: toTagColor(colorMap[key] ?? '#666') }, { default: () => label })
}
...
const optionLogColumns = computed<DataTableColumns<Recordable>>(() => [
  ...
  },
  {
    ...
    width: 100,
    render(row) {
      return h(NTag, { size: 'small', type: row.success ? 'success' : 'error' }, {
  ...
  { title: t('flink.app.detail.columns.optionTime'), key: 'optionTime', width: 180 },
  {
    ...
    render(row) {
      ...
      if (!row.success) {
        actions.push(actionBtn(I.view, t('spark.app.detail.detailTab.exception'), () => {
          execOptionContent.value = row.exception ?? ''
          execOptionVisible.value = true
        }))
      }
      actions.push(
        h(NPopconfirm, { onPositiveClick: () => handleDeleteLog(row.id) }, {
  ...
  { title: t('flink.app.detail.columns.modifyTime'), key: 'createTime', width: 180 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          actionBtn(I.view, t('spark.app.detail.detailTab.sqlDetail'), () => openSqlView(row)),
          actionBtn(I.compare, t('spark.app.detail.compareSparkSql'), () => openSqlCompare(row), sparkSqlVersions.value.length <= 1),
          h(NPopconfirm, { onPositiveClick: () => handleDeleteSparkSql(row) }, {
            trigger: () => actionBtn(I.delete, t('common.delText'), () => {}, !!row.effective),
...
const confColumns = computed<DataTableColumns<Recordable>>(() => [
  {
    ...
    key: 'version',
    width: 90,
    render: row => h(NTag, { size: 'small', type: 'primary', round: true }, { default: () => String(row.version) }),
  },
  {
    title: 'Format',
    key: 'format',
    ...
    render(row) {
      if (row.format === ConfigTypeEnum.YAML)
        return h(NTag, { size: 'small', color: toTagColor('#2db7f5') }, { default: () => 'yaml' })
      if (row.format === ConfigTypeEnum.PROPERTIES)
        return h(NTag, { size: 'small', color: toTagColor('#108ee9') }, { default: () => 'properties' })
      return '-'
    },
  },
  {
  ...
  { title: t('flink.app.detail.columns.modifyTime'), key: 'createTime', width: 180 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          actionBtn(I.view, t('spark.app.detail.detailTab.configDetail'), () => openConfView(row)),
          actionBtn(I.compare, t('spark.app.detail.compareConfig'), () => openConfCompare(row), confVersions.value.length <= 1),
          h(NPopconfirm, { onPositiveClick: () => handleDeleteConf(row) }, {
            trigger: () => actionBtn(I.delete, t('common.delText'), () => {}, !!row.effective),
...
const backupColumns = computed<DataTableColumns<Recordable>>(() => [
  ...
  { title: t('flink.app.detail.columns.modifyTime'), key: 'createTime', width: 180 },
  {
    ...
    render(row) {
      return h(NSpace, { size: 4 }, {
        default: () => [
          actionBtn(I.copy, t('spark.app.detail.detailTab.copyPath'), () => handleCopyPath(row.path)),
          h(NPopconfirm, { onPositiveClick: () => handleDeleteBackup(row.id) }, {
            trigger: () => actionBtn(I.delete, t('common.delText'), () => {}),
...
async function checkTabVisibility() {
  ...
  showBackupTab.value = backupResult.isSuccess && Array.isArray(backupResult.data) && backupResult.data.length > 0
}
...
async function loadSparkSqlVersions() {
  ...
  try {
    ...
    if (result.isSuccess)
      sparkSqlVersions.value = resolveListData(result.data).records
  }
  catch {
...
async function loadConfVersions() {
  ...
  try {
    ...
    if (result.isSuccess)
      confVersions.value = resolveListData<Recordable>(result.data).records
  }
  catch {
    window.$message?.error(t('sys.api.apiRequestFailed'))
  }
...
async function loadBackups() {
  ...
  try {
    ...
    if (result.isSuccess)
      backupRecords.value = (result.data as Recordable[]) ?? []
  }
  catch {
    window.$message?.error(t('sys.api.apiRequestFailed'))
  }
...
async function loadOptionLogs() {
  ...
  try {
    ...
    if (result.isSuccess)
      optionLogs.value = (result.data as Recordable[]) ?? []
  }
  catch {
    window.$message?.error(t('sys.api.apiRequestFailed'))
  }
...
async function loadApp() {
  ...
  try {
    ...
    if (Object.keys(app).length > 0)
      await loadYarnIfNeeded(result.data)
  ...
  <n-card :bordered="false" class="h-full">
    <n-spin :show="loading">
      <div class="mb-16px flex flex-wrap items-center justify-between gap-12px">
        <span class="text-18px font-medium">{{ t('spark.app.detail.detailTitle') }} — {{ app.appName }}</span>
        <n-space>
      ...
      <n-tabs v-model:value="activeTab" type="line" animated>
        ...
        <n-tab-pane name="optionLog" :tab="t('spark.app.detail.detailTab.detailTabName.operationLog')">
          <n-data-table
            :loading="tabLoading"
        ...
        <n-tab-pane v-if="isSqlApp" name="sparkSql" :tab="t('spark.app.detail.detailTab.detailTabName.sparkSql')">
          <n-data-table
            :loading="tabLoading"
