...
export function handleIsStart(
    app: SparkApplication,
    optionApps: { starting: Map<string, number> },
) {
    const status = [
        AppStateEnum.ADDED,
        AppStateEnum.FAILED,
    ...
    const release = [ReleaseStateEnum.DONE, ReleaseStateEnum.NEED_RESTART].includes(
        app.release as number,
    )
    const optionState =
        !optionApps.starting.get(app.id!) || app.optionState === OptionStateEnum.NONE
...
export function canCancelSpark(app: SparkApplication) {
    return (
        [AppStateEnum.ACCEPTED, AppStateEnum.RUNNING, AppStateEnum.SUBMITTED].includes(
            app.state as AppStateEnum,
