...
function updateTitleLens(records: SparkApplication[]) {
  ...
  records.forEach((cur) => {
    ...
    if (optionState === OptionStateEnum.NONE) {
      ...
      if (stateStr && stateStr.length > lens.maxState.length)
        lens.maxState = stateStr
    }
    else {
    ...
    if (releaseStr && releaseStr.length > lens.maxRelease.length)
      lens.maxRelease = releaseStr
    const buildStr = buildStatus != null ? sparkBuildStatusMap[buildStatus]?.title : undefined
    if (buildStr && buildStr.length > lens.maxBuild.length)
      lens.maxBuild = buildStr
  })
...
function cleanupOptionApps(records: SparkApplication[]) {
  ...
  records.forEach((row) => {
    if (row.optionState !== OptionStateEnum.NONE || !row.id)
      return
...
async function submitCopy() {
  ...
  if (result.isSuccess && ok) {
    window.$message?.success('copy successful')
    copyModalVisible.value = false
    handlePageDataReload(false)
  }
  else {
    const msg = (message || 'copy failed').replaceAll(/\[StreamPark]/g, '')
...
async function submitMapping() {
  ...
  if (!clusterId) {
    window.$message?.warning('jobId is required')
    return
  }
  ...
  if (result.isSuccess) {
    window.$message?.success('The current job is mapping')
...
function buildRowActions(row: SparkApplication) {
  ...
  if (hasPermission('app:update')) {
    actions.push(actionBtn(I.edit, t('spark.app.operation.edit'), () => {
  ...
  if (hasPermission('app:release') && row.release != null && [
    ...
    actions.push(actionBtn(I.release, t('spark.app.operation.release'), () => handleRelease(row), 'e2e-sparkapp-release-btn'))
  }
  if (hasPermission('app:release') && (
    (row.release != null && [ReleaseStateEnum.FAILED, ReleaseStateEnum.RELEASING].includes(row.release))
    || row.optionState === OptionStateEnum.RELEASING
  )) {
    actions.push(actionBtn(I.releaseDetail, t('spark.app.operation.releaseDetail'), () => {
  ...
  if (hasPermission('app:start') && handleIsStart(row, optionApps)) {
    actions.push(actionBtn(I.start, t('spark.app.operation.start'), () => handleStartCheck(row), 'e2e-sparkapp-startup-btn'))
  }
  if (hasPermission('app:cancel') && canCancelSpark(row)) {
    actions.push(actionBtn(I.pause, t('spark.app.operation.cancel'), () => confirmCancel(row), 'e2e-sparkapp-cancel-btn'))
  }
  if (hasPermission('app:detail')) {
    actions.push(actionBtn(I.detail, t('spark.app.operation.detail'), () => {
  ...
  if (dropdownOptions.length) {
    actions.push(
      h(NDropdown, {
        ...
        default: () => h(NButton, { quaternary: true, size: 'small', class: 'e2e-sparkapp-more-btn' }, { default: () => '...' }),
      }),
    )
  }
...
function handleColumnWidthChange(width: number, _limitedWidth: number, column: { key?: string | number }) {
  if (column.key === 'appName')
    appNameColumnWidth.value = width
...
const columns = computed<DataTableColumns<SparkApplication>>(() => [
  ...
  },
  {
    ...
    key: 'state',
    width: 130,
    render: row => h(SparkAppStateTag, { option: 'state', data: row, maxTitle: titleLenRef.maxState }),
  },
  {
    ...
    minWidth: 130,
    resizable: true,
    render: row => h(SparkAppStateTag, { option: 'release', data: row, maxTitle: titleLenRef.maxRelease }),
  },
  {
    title: 'Build',
    key: 'buildStatus',
    width: 110,
    render: row => h(SparkAppStateTag, { option: 'build', data: row, maxTitle: titleLenRef.maxBuild }),
  },
  {
...
async function loadData(polling = false) {
  ...
  try {
    ...
    if (searchForm.stateArray.length > 0)
      params.stateArray = [...searchForm.stateArray]
...
async function ensureYarnUrl() {
  if (yarnUrl.value != null)
    return
  const result = await fetchSparkYarn()
  if (result.isSuccess)
    yarnUrl.value = result.data ?? null
}

async function handleJobView(app: SparkApplication) {
  if (app.state !== AppStateEnum.RUNNING)
    return
  await ensureYarnUrl()
    ...
    <n-card :bordered="false" class="flex-1">
      <div class="mb-16px flex flex-wrap items-end justify-between gap-12px">
        ...
        <n-space>
          ...
          <n-button id="e2e-sparkapp-create-btn" v-auth="'app:create'" type="primary" @click="router.push('/spark/app/add')">
            {{ t('common.add') }}
          </n-button>
        </n-space>
      </div>
      ...
      <n-modal v-model:show="copyModalVisible" preset="card" :style="{ width: '480px' }" title="Copy Application">
        <n-form label-placement="top">
          <n-form-item label="Job Name" required>
            <n-input v-model:value="copyAppName" placeholder="New Job Name" />
          </n-form-item>
        </n-form>
        <template #footer>
          <n-space justify="end">
      ...
      <n-modal v-model:show="mappingModalVisible" preset="card" :style="{ width: '520px' }" title="Mapping Application">
        <n-form label-placement="top">
          ...
          <n-form-item
            v-if="mappingApp && [DeployMode.YARN_CLIENT, DeployMode.YARN_CLUSTER].includes(mappingApp.deployMode!)"
            label="YARN Application Id"
            required
          >
            <n-input v-model:value="mappingForm.clusterId" placeholder="ApplicationId" />
          </n-form-item>
          <n-form-item label="JobId" required>
            <n-input v-model:value="mappingForm.jobId" placeholder="JobId" />
          </n-form-item>
        </n-form>
        <template #footer>
          <n-space justify="end">
            ...
            <n-button type="primary" class="e2e-sparkapp-remapping-btn" @click="submitMapping">
              {{ t('common.apply') }}
            </n-button>
          </n-space>
        </template>
