...
async function validateAppName(_rule: FormItemRule, value: string) {
    if (!value?.trim()) return Promise.reject(t('spark.app.addAppTips.appNameIsRequiredMessage'))
    const params: { appName: string; id?: string } = { appName: value }
    if (props.appId) params.id = props.appId
    const result = await fetchCheckSparkName(params)
    if (!result.isSuccess) return Promise.reject(t('spark.app.addAppTips.appNameValid'))
...
const rules: FormRules = {
    ...
    versionId: [
        {
            required: true,
            message: t('spark.app.addAppTips.sparkVersionIsRequiredMessage'),
            trigger: 'change',
        },
    ],
    appName: [{ required: true, validator: validateAppName, trigger: 'blur' }],
    jar: [
        { required: true, message: t('spark.app.addAppTips.sparkAppRequire'), trigger: 'change' },
    ],
    mainClass: [
        {
            required: true,
            message: t('spark.app.addAppTips.mainClassIsRequiredMessage'),
            trigger: 'blur',
        },
    ],
}
...
async function loadOptions() {
    ...
    if (envResult.isSuccess && envResult.data) sparkEnvs.value = envResult.data
    if (resourceResult.isSuccess && resourceResult.data)
        teamResources.value = resourceResult.data as typeof teamResources.value
    if (queueResult.isSuccess) yarnQueues.value = resolveListData(queueResult.data).records
...
async function handleSubmit() {
    ...
    try {
        ...
        if (values.isSetConfig && values.configOverride?.trim()) config = values.configOverride
        else config = null
    ...
    >
        <n-grid :cols="24" :x-gap="16">
            <n-form-item-gi v-if="mode === 'create'" :span="12" :label="t('spark.app.jobType')">
                <n-select v-model:value="formModel.jobType" :options="jobTypeOptions" />
            </n-form-item-gi>
            ...
            <n-form-item-gi :span="12" :label="t('spark.app.deployMode')" path="deployMode">
                <n-select v-model:value="formModel.deployMode" :options="deployModes" />
            </n-form-item-gi>
            ...
            <n-form-item-gi :span="12" :label="t('spark.app.appName')" path="appName">
                <n-input
                    ...
                    :placeholder="t('spark.app.addAppTips.appNamePlaceholder')"
                />
            </n-form-item-gi>
            <n-form-item-gi
                v-if="isSqlJob"
                :span="24"
                :label="t('spark.app.detail.detailTab.detailTabName.sparkSql')"
            >
                <SparkSqlEditor
                    ref="sqlEditorRef"
            ...
            <template v-if="isJarJob">
                <n-form-item-gi :span="12" :label="t('spark.app.resource')" path="jar">
                    <n-select v-model:value="formModel.jar" :options="jarOptions" filterable />
                </n-form-item-gi>
                <n-form-item-gi :span="12" :label="t('spark.app.mainClass')" path="mainClass">
                    <n-input v-model:value="formModel.mainClass" />
                </n-form-item-gi>
            </template>
            <n-form-item-gi v-if="!isSqlJob" :span="24" :label="t('spark.app.programArgs')">
                <ProgramArgsEditor
