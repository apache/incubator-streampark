...
import { fetchSparkSqlVerify, fetchVariableAll } from '@/service'
...
async function handleVerifySql() {
  if (!props.modelValue?.trim()) {
    verifyError.value = 'empty sql'
    return false
  }
  if (!props.versionId) {
    window.$message?.error(t('spark.app.addAppTips.sparkVersionIsRequiredMessage'))
    return false
  }
  verifying.value = true
  verifyError.value = ''
  try {
    const result = await fetchSparkSqlVerify({
    ...
    const success = payload?.data === true || payload?.data === 'true'
    if (success) {
      window.$message?.success(t('spark.app.sparkSql.successful'))
      return true
    }
    verifyError.value = payload?.message || t('sys.api.apiRequestFailed')
    return false
  }
  catch (e: any) {
...
<template>
  <Teleport to="body" :disabled="!fullscreen">
    <div
      ...
      :class="fullscreen ? 'fixed inset-0 z-[2000] flex flex-col bg-[var(--body-color)] p-16px' : ''"
    >
      <div v-if="fullscreen" class="mb-8px flex shrink-0 items-center justify-between">
        <span class="font-medium">{{ t('spark.app.detail.detailTab.detailTabName.sparkSql') }}</span>
      ...
      <div v-if="!readonly" class="mb-8px flex shrink-0 flex-wrap justify-end gap-8px">
        ...
        <n-button size="small" @click="toggleFullscreen">
          <template #icon>
            <n-icon><IonIcon :name="fullscreen ? 'ContractOutline' : 'ExpandOutline'" /></n-icon>
          </template>
          {{ fullscreen ? t('spark.app.sparkSql.exit') : t('spark.app.sparkSql.fullScreen') }}
        </n-button>
        <n-button size="small" type="primary" :loading="verifying" @click="handleVerifySql">
      ...
      <VariableReviewDrawer
        ...
        :title="t('spark.app.sparkSql.preview')"
      />
    </div>
  </Teleport>
</template>
