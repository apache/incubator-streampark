...
async function handleSubmit() {
  ...
  try {
    ...
    const inner = result.data as { data?: boolean, msg?: string, message?: string } | boolean | undefined
    const success = typeof inner === 'boolean' ? inner : Boolean(inner?.data)
    if (!success) {
      const message = (typeof inner === 'object' ? (inner?.msg || inner?.message) : result.message) || t('sys.api.apiRequestFailed')
