...
import { checkResource, fetchAddResource, fetchUpdateResource, fetchUpload } from '@/service'
...
const rules = computed<FormRules>(() => {
    const base: FormRules = {
        engineType: [
            {
                required: true,
                message: t('flink.resource.form.engineTypeIsRequiredMessage'),
                trigger: 'change',
            },
        ],
        resourceType: [
            {
                required: true,
                message: t('flink.resource.form.resourceTypeIsRequiredMessage'),
                trigger: 'change',
            },
        ],
        description: [
            { max: 100, message: t('flink.resource.form.descriptionMessage'), trigger: 'blur' },
        ],
    }
    if (showResourceName.value) {
        base.resourceName = [
            {
                required: true,
                message: t('flink.resource.form.resourceNameIsRequiredMessage'),
                trigger: 'blur',
            },
        ]
    }
    if (showGroupName.value) {
        base.resourceName = [
            {
                required: true,
                message: t('flink.resource.groupNameIsRequiredMessage'),
                trigger: 'blur',
            },
        ]
    }
...
async function handleJarUpload(options: UploadCustomRequestOptions) {
    ...
    try {
        ...
        if (!path) throw new Error(t('flink.resource.jarFileErrorTip'))
    ...
    switch (state) {
        ...
        case 3:
            window.$message?.error(
                `${t('flink.resource.connectorInfoErrorTip')}: ${resp.name ?? ''}`,
            )
            break
        case 4:
        ...
        <n-drawer-content :title="drawerTitle" closable>
            <n-form ref="formRef" :model="formModel" :rules="rules" label-placement="top">
                ...
                <n-form-item :label="t('flink.resource.resourceType')" path="resourceType">
                    <n-select
                        v-model:value="formModel.resourceType"
                ...
                >
                    <n-input
                        ...
                        :placeholder="t('flink.resource.resourceNamePlaceholder')"
                    />
                </n-form-item>
                <n-form-item
                    v-if="showGroupName"
                ...
                >
                    <n-input
                        v-model:value="formModel.resourceName"
                        :placeholder="t('flink.resource.groupNamePlaceholder')"
                    />
                </n-form-item>
                <n-form-item
                    v-if="showGroupName"
