...
function renderResourceType(row: ResourceListRecord) {
  ...
  return h(NTag, { type: 'info', size: 'small' }, {
    default: () => h('span', { class: 'inline-flex items-center gap-4px' }, [
  ...
  { title: t('common.modifyTime'), key: 'modifyTime', sorter: true },
  {
    ...
    render(row) {
      ...
      if (hasPermission('resource:update')) {
        actions.push(actionBtn(I.edit, t('flink.resource.modifyResource'), () => openEdit(row), 'e2e-upload-edit-btn'))
      }
      if (hasPermission('resource:delete')) {
        actions.push(h(NPopconfirm, { onPositiveClick: () => handleDelete(row) }, {
          trigger: () => actionBtn(I.delete, t('flink.resource.deleteResource'), undefined, 'e2e-upload-delete-btn'),
...
async function loadTeamResource() {
  ...
  if (result.isSuccess)
    teamResource.value = result.data ?? []
}
  ...
  <n-card :bordered="false" class="h-full">
    <div class="mb-16px flex flex-wrap items-center justify-between gap-12px">
      ...
      <n-space>
        ...
        <n-button id="e2e-upload-create-btn" v-auth="'resource:add'" type="primary" @click="openCreate">
          {{ t('common.add') }}
        </n-button>
      </n-space>
    </div>
