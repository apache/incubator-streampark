...
const repositoryOptions = [
  { label: 'GitHub/GitLab', value: CVSTypeEnum.GIT },
]
...
async function handleSubmit() {
  ...
  try {
    ...
    const result = await fetchProjectCreate(buildPayload())
    if (!result.isSuccess || !result.data)
      throwApiFailure(result, t('flink.project.saveFailed'))

    window.$message?.success(t('flink.project.createSuccess'))
...
<template>
  <n-card :bordered="false" class="h-full">
    ...
    <div class="mt-24px flex justify-center gap-12px">
      ...
      <n-button id="e2e-project-submit-btn" type="primary" :loading="submitting" @click="handleSubmit">
        {{ t('common.submitText') }}
      </n-button>
    </div>
  </n-card>
