...
function renderProjectType(row: ProjectRecord) {
  ...
  if (row.buildState === BuildStateEnum.NEED_REBUILD) {
    return h(NBadge, { value: 'NEW', type: 'warning' }, {
      default: () => h('span', { class: 'inline-flex items-center gap-6px' }, [iconEl, meta.label]),
    })
  }
  ...
  },
  {
    ...
    render(row) {
      ...
      if (hasPermission('project:build')) {
        actions.push(actionBtn(I.code, t('flink.project.operationTips.seeBuildLog'), () => openLog(row)))
        if (!isBuilding) {
          actions.push(h(NPopconfirm, {
            onPositiveClick: () => handleBuild(row),
          }, {
            trigger: () => actionBtn(I.build, t('flink.project.operationTips.buildProject'), undefined, 'e2e-project-build-btn'),
      ...
      if (!isBuilding && hasPermission('project:delete')) {
        actions.push(h(NPopconfirm, { onPositiveClick: () => handleDelete(row) }, {
          trigger: () => actionBtn(I.delete, t('common.delText'), undefined, 'e2e-project-delete-btn'),
...
async function loadData(silent = false) {
  ...
  try {
    ...
    const { records, total } = resolveListData(result.data as ProjectRecord[] | { records?: ProjectRecord[], total?: number })
...
async function handleDelete(record: ProjectRecord) {
  try {
    ...
    if (!result.isSuccess || !result.data)
      throwApiFailure(result, t('flink.project.operationTips.deleteProjectFailedDetailMessage'))
...
<template>
  <n-card :bordered="false" class="h-full">
    <div class="mb-16px flex flex-wrap items-center justify-between gap-12px">
      ...
      <n-button id="e2e-project-create-btn" v-auth="'project:create'" type="primary" @click="router.push('/project/add')">
        {{ t('common.add') }}
      </n-button>
    </div>
    <n-data-table
