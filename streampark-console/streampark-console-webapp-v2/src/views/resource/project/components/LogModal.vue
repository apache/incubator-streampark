...
const modalTitle = computed(() =>
    props.project
        ? `${props.project.name} Build Log`
        : t('flink.project.operationTips.seeBuildLog'),
)
...
async function refreshLog() {
    ...
    try {
        ...
        if (payload.readFinished === false) startPolling()
        else stopPolling()
        if (payload.data) logText.value = String(payload.data)
...
<template>
    ...
    >
        ...
        <n-spin :show="loading">
            <pre
                class="log-content max-h-500px min-h-300px overflow-auto whitespace-pre-wrap break-all"
                >{{ logText }}</pre>
        </n-spin>
        <template #footer>
