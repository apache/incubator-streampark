...
const columns = computed<DataTableColumns<VariableListRecord>>(() => [
  ...
  { title: t('flink.variable.table.variableValue'), key: 'variableValue', ellipsis: { tooltip: true } },
  ...
  { title: t('common.modifyTime'), key: 'modifyTime', sorter: true },
  {
    ...
    render(row) {
      ...
      if (hasPermission('variable:update')) {
        actions.push(actionBtn(I.edit, t('flink.variable.modifyVariable'), () => openEdit(row), 'e2e-var-edit-btn'))
      }
      actions.push(actionBtn(I.detail, t('common.detail'), () => openDetail(row)))
      if (hasPermission('variable:depend_apps')) {
        actions.push(actionBtn(I.depend, t('flink.variable.table.depend'), () =>
      ...
      if (hasPermission('variable:delete')) {
        actions.push(h(NPopconfirm, { onPositiveClick: () => handleDelete(row) }, {
          trigger: () => actionBtn(I.delete, t('flink.variable.deleteVariable'), undefined, 'e2e-var-delete-btn'),
  ...
  <n-card :bordered="false" class="h-full">
    <div class="mb-16px flex flex-wrap items-center justify-between gap-12px">
      ...
      <n-space>
        ...
        <n-button id="e2e-var-create-btn" v-auth="'variable:add'" type="primary" @click="openCreate">
          {{ t('common.add') }}
        </n-button>
      </n-space>
    </div>
