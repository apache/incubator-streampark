...
async function toggleValue() {
    if (!props.record?.desensitization || !hasPermission('variable:show_original')) return
    if (!realValue.value) {
        const result = await fetchVariableInfo({ id: String(props.record.id) })
        if (result.isSuccess && result.data) realValue.value = result.data.variableValue
    }
                        ...
                        <template #icon>
                            <n-icon
                                ><component
                                    :is="
                                        showOriginal
                                            ? ionIconComponent('EyeOffOutline')
                                            : ionIconComponent('EyeOutline')
                                    "
                            /></n-icon>
                        </template>
                    </n-button>
