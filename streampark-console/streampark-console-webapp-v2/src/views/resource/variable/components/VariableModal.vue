  ...
  variableCode: props.isUpdate
    ...
    : [{
        ...
        asyncValidator: async (_rule, value: string) => {
          if (!value)
            throw new Error(t('flink.variable.form.empty'))
          if (value.length < 3 || value.length > 50)
            throw new Error(t('flink.variable.form.len'))
          if (!codePattern.test(value))
            throw new Error(t('flink.variable.form.regExp'))
          ...
          if (!result.data?.data)
            throw new Error(t('flink.variable.form.exists'))
        },
      }],
  ...
  >
    ...
    <template #footer>
      <n-space justify="end">
        ...
        <n-button class="e2e-var-submit-btn" type="primary" :loading="submitting" @click="handleSubmit">
          {{ t('common.submitText') }}
        </n-button>
      </n-space>
    </template>
