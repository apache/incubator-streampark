...
const rules = computed<FormRules>(() => ({
  queueLabel: [{
    ...
    asyncValidator: async (_rule, value: string) => {
      ...
      if (props.isUpdate && formModel.value.id)
        payload.id = formModel.value.id
      ...
      if (status === 0)
        return
      if (status === 1)
        throw new Error(t('setting.yarnQueue.checkResult.existedHint'))
      if (status === 2)
        throw new Error(t('setting.yarnQueue.checkResult.invalidFormatHint'))
      throw new Error(t('setting.yarnQueue.checkResult.emptyHint'))
    },
  }],
  description: [{ max: 512, message: t('setting.yarnQueue.descriptionMessage'), trigger: 'blur' }],
}))
