...
const rules = computed<FormRules>(() => {
  const base: FormRules = {
    alertName: [{
      ...
      asyncValidator: async (_rule, value: string) => {
        if (!value)
          throw new Error(t('setting.alarm.alertNameErrorMessage.alertNameIsRequired'))
        if (!alertId.value) {
          const result = await fetchExistsAlert({ alertName: value })
          if (!result.isSuccess)
            throwApiFailure(result, t('setting.alarm.alertNameErrorMessage.alertConfigFailed'))
          if (result.data)
            throw new Error(t('setting.alarm.alertNameErrorMessage.alertNameAlreadyExists'))
        }
      },
    }],
    alertType: [{
  ...
  if (hasType('1')) {
    base.alertEmail = [
      { required: true, message: t('setting.alarm.alertEmailAddressIsRequired'), trigger: 'blur' },
      { pattern: emailPattern, message: t('setting.alarm.alertEmailFormatIsInvalid'), trigger: 'blur' },
    ]
  }

  if (hasType('2')) {
    base.alertDingURL = [
      { pattern: dingTalkUrlPattern, message: t('setting.alarm.dingTalkUrlFormatIsInvalid'), trigger: 'blur' },
    ]
    base.dingtalkToken = [
    ...
    if (formModel.value.dingtalkSecretEnable) {
      base.dingtalkSecretToken = [
        { required: true, message: t('setting.alarm.dingTalkSecretTokenIsRequired'), trigger: 'blur' },
      ]
    }
  }
  ...
  if (hasType('16')) {
    base.larkToken = [
      { required: true, message: 'Lark token is required', trigger: 'blur' },
    ]
    if (formModel.value.larkSecretEnable) {
      base.larkSecretToken = [
        { required: true, message: t('setting.alarm.larkSecretTokenIsRequired'), trigger: 'blur' },
      ]
    }
  }
...
async function handleSubmit() {
  ...
  try {
    ...
    if (!isEdit.value) {
      ...
      if (!exists.isSuccess)
        throwApiFailure(exists, t('sys.api.apiRequestFailed'))
      if (exists.data) {
        window.$dialog?.error({
    ...
    if (!ok) {
      ...
      window.$dialog?.error({
        title: isEdit.value ? t('setting.alarm.fail.update') : t('setting.alarm.fail.title'),
        content: message,
        positiveText: t('common.okText'),
      })
      return
