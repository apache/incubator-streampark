...
function lookupIonComponent(name: string): Component | undefined {
    if (ION_ICON_REGISTRY[name]) return ION_ICON_REGISTRY[name]
...
export function resolveShortIcon(icon: string): string {
    if (SHORT_ICON_ALIASES[icon]) return SHORT_ICON_ALIASES[icon]
    if (LEGACY_ICON_ALIASES[icon]) return LEGACY_ICON_ALIASES[icon]
    if (VICON_TO_ICONIFY[icon]) return VICON_TO_ICONIFY[icon]

    if (icon.endsWith('-outlined') || icon.endsWith('-filled') || icon.endsWith('-twotone')) {
        const base = icon.replace(/-(outlined|filled|twotone)$/, '')
        if (SHORT_ICON_ALIASES[base]) return SHORT_ICON_ALIASES[base]
        return `ant-design:${icon}`
    }

    if (/^[A-Z]/.test(icon) && lookupIonComponent(icon)) return ''
...
export function resolveViconComponent(icon?: string): Component | undefined {
    ...
    if (icon.includes(':') && !icon.startsWith('ion:')) return undefined

    if (VICON_TO_ICONIFY[icon]) return undefined
    ...
    if (!ionRef) return lookupIonComponent(icon)
...
export function resolveIconifyRef(icon?: string): string {
    if (!icon || icon.endsWith('|svg')) return icon ?? ''

    if (icon.includes(':')) return icon

    if (VICON_TO_ICONIFY[icon]) return VICON_TO_ICONIFY[icon]

    if (lookupIonComponent(icon)) return ''
