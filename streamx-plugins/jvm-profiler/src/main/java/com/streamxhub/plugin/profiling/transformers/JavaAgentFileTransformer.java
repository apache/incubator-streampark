...
package com.streamxhub.plugin.profiling.transformers;
...
import com.streamxhub.plugin.profiling.util.ClassMethodArgumentFilter;
    ...
    private void transformMethod(String normalizedClassName, CtMethod method, boolean enableDurationProfiling, List<Integer> argumentsForProfile) {
        ...
        try {
            ...
            for (Integer argument : argumentsForProfile) {
                if (argument >= 1) {
                    sb.append(String.format("try{com.streamxhub.plugin.profiling.transformers.MethodProfilerStaticProxy.collectMethodArgument(\"%s\", \"%s\", %s, String.valueOf($%s));}catch(Throwable ex){ex.printStackTrace();}",
                            normalizedClassName,
                            method.getName(),
                            argument,
                            argument));
                } else {
                    sb.append(String.format("try{com.streamxhub.plugin.profiling.transformers.MethodProfilerStaticProxy.collectMethodArgument(\"%s\", \"%s\", %s, \"\");}catch(Throwable ex){ex.printStackTrace();}",
                            normalizedClassName,
                            method.getName(),
                            argument,
                            argument));
            ...
            if (enableDurationProfiling) {
                method.insertAfter("{" +
                        ...
                        String.format("try{com.streamxhub.plugin.profiling.transformers.MethodProfilerStaticProxy.collectMethodDuration(\"%s\", \"%s\", durationMillis_java_agent_instrument);}catch(Throwable ex){ex.printStackTrace();}", normalizedClassName, method.getName()) +
