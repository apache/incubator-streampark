...
public final class SparkClientEndpoint {
    ...
    public static SubmitResponse submit(SubmitRequest submitRequest) throws Exception {
        SparkClientTrait client = CLIENTS.get(submitRequest.deployMode());
        if (client == null) {
            throw new UnsupportedOperationException(
                "Unsupported " + submitRequest.deployMode() + " spark submit.");
        }
        return client.submit(submitRequest);
    }

    public static CancelResponse cancel(CancelRequest stopRequest) throws Exception {
        SparkClientTrait client = CLIENTS.get(stopRequest.deployMode());
        if (client == null) {
            throw new UnsupportedOperationException(
                "Unsupported " + stopRequest.deployMode() + " spark stop.");
        }
