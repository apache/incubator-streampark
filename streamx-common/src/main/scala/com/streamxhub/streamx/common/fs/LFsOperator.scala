...
object LFsOperator extends FsOperator with Logger {
  ...
  override def download(srcPath: String, dstPath: String, delSrc: Boolean, useRawLocalFileSystem: Boolean): Unit = copyDir(srcPath, dstPath,delSrc=false, overwrite = true)
}
