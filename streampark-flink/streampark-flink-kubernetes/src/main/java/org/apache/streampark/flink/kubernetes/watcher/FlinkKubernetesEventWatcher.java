 ...
 */
@ThreadSafe
public class FlinkKubernetesEventWatcher extends FlinkWatcher {

    private final FlinkKubernetesWatchController watchController;

    private KubernetesClient k8sClient;

    public FlinkKubernetesEventWatcher(FlinkKubernetesWatchController watchController) {
    ...
    private void handleDeploymentEvent(Watcher.Action action, Deployment event) {
        ...
        watchController.k8sDeploymentEvents.put(
            KubernetesEventKey.builder().namespace(namespace).clusterId(clusterId).build(),
            KubernetesDeploymentEventCV.builder()
                .action(action)
                .event(event)
