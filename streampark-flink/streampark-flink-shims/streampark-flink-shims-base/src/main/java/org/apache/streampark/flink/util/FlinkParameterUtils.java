...
public final class FlinkParameterUtils {
    ...
    public static String getAppName(FlinkJobParameters parameterTool) {
        return getAppName(parameterTool, null, false);
    }

    public static String getAppName(FlinkJobParameters parameterTool, boolean required) {
        return getAppName(parameterTool, null, required);
    }

    public static String getAppName(
                                    FlinkJobParameters parameterTool, @Nullable String name, boolean required) {
        String appName;
        if (name == null) {
            appName = null;
            String zippedAppName =
                parameterTool.getOptional(ApplicationOptions.NAME).orElse(null);
            ...
            if (appName == null) {
                appName = parameterTool.getOptional(FlinkOptions.PIPELINE_NAME).orElse(null);
            }
        } else {
            appName = name;
        }
    ...
    public static <T extends TableEnvironment> T setAppName(T env, FlinkJobParameters parameter) {
    ...
    public static StreamTableEnvironment setAppName(
                                                    StreamTableEnvironment env, FlinkJobParameters parameter) {
        String appName = getAppName(parameter);
        if (appName != null) {
            env.getConfig().getConfiguration().setString(PipelineOptions.NAME.key(), appName);
        }
