...
import org.apache.flink.contrib.streaming.state.{ConfigurableRocksDBOptionsFactory, DefaultConfigurableOptionsFactory, EmbeddedRocksDBStateBackend}
...
import org.apache.streampark.common.conf.ConfigConst.{KEY_APP_CONF, KEY_EXECUTION_RUNTIME_MODE, KEY_FLINK_CHECKPOINTS_CLEANUP, KEY_FLINK_CHECKPOINTS_ENABLE, KEY_FLINK_CHECKPOINTS_INTERVAL, KEY_FLINK_CHECKPOINTS_MAX_CONCURRENT, KEY_FLINK_CHECKPOINTS_MIN_PAUSEBETWEEN, KEY_FLINK_CHECKPOINTS_MODE, KEY_FLINK_CHECKPOINTS_TIMEOUT, KEY_FLINK_CHECKPOINTS_UNALIGNED, KEY_FLINK_CONF, KEY_FLINK_DEPLOYMENT_OPTION_PREFIX, KEY_FLINK_PARALLELISM, KEY_FLINK_RESTART_STRATEGY, KEY_FLINK_RESTART_STRATEGY_FAILURE_RATE_DELAY, KEY_FLINK_RESTART_STRATEGY_FAILURE_RATE_PER_INTERVAL, KEY_FLINK_RESTART_STRATEGY_FAILURE_RATE_RATE_INTERVAL, KEY_FLINK_RESTART_STRATEGY_FIXED_DELAY_ATTEMPTS, KEY_FLINK_RESTART_STRATEGY_FIXED_DELAY_DELAY, KEY_FLINK_STATE_BACKEND, KEY_FLINK_STATE_BACKEND_ASYNC, KEY_FLINK_STATE_BACKEND_INCREMENTAL, KEY_FLINK_STATE_BACKEND_MEMORY, KEY_FLINK_STATE_CHECKPOINTS_DIR, KEY_FLINK_STATE_CHECKPOINT_STORAGE, KEY_FLINK_STATE_ROCKSDB, KEY_FLINK_WATERMARK_INTERVAL}
...
import org.apache.streampark.common.util.{DateUtils, DeflaterUtils, HdfsUtils, Logger, PropertiesUtils}
...
private[flink] class FlinkStreamingInitializer(args: Array[String], apiType: ApiType) extends Logger {
  ...
  private[this] lazy val defaultFlinkConf: Map[String, String] = {
    Option(parameter.get(KEY_FLINK_CONF())) match {
      case None =>
      ...
      case Some(conf) => PropertiesUtils.loadFlinkConfYaml(DeflaterUtils.unzipString(conf))
    }
  }
  ...
  def initStreamEnv(): Unit = {
    ...
    val executionMode = Try(RuntimeExecutionMode.valueOf(parameter.get(KEY_EXECUTION_RUNTIME_MODE)))
      .getOrElse(RuntimeExecutionMode.STREAMING)
  ...
  def readFlinkConf(config: String): Map[String, String] = {
    ...
    (config match {
      case x if x.startsWith("yaml://") => PropertiesUtils.fromYamlText(DeflaterUtils.unzipString(x.drop(7)))
      case x if x.startsWith("prop://") => PropertiesUtils.fromPropertiesText(DeflaterUtils.unzipString(x.drop(7)))
    ...
    .map(x => x._1.replace(KEY_FLINK_DEPLOYMENT_OPTION_PREFIX, "") -> x._2)
  }

  def restartStrategy(): Unit = {
    val prefixLen = "flink".length
    val strategy = Try(RestartStrategy.byName(parameter.get(KEY_FLINK_RESTART_STRATEGY)))
      .getOrElse(Try(RestartStrategy.byName(defaultFlinkConf("restart-strategy"))).getOrElse(null))
    strategy match {
      case RestartStrategy.`failure-rate` =>
        val interval = Try(parameter.get(KEY_FLINK_RESTART_STRATEGY_FAILURE_RATE_PER_INTERVAL).toInt)
          .getOrElse(
            Try(defaultFlinkConf(KEY_FLINK_RESTART_STRATEGY_FAILURE_RATE_PER_INTERVAL).drop(prefixLen).toInt)
              .getOrElse(3)
          )
      ...
      case RestartStrategy.`fixed-delay` =>
        val attempts = Try(parameter.get(KEY_FLINK_RESTART_STRATEGY_FIXED_DELAY_ATTEMPTS).toInt)
          .getOrElse(
            Try(defaultFlinkConf(KEY_FLINK_RESTART_STRATEGY_FIXED_DELAY_ATTEMPTS.drop(prefixLen)).toInt)
              .getOrElse(3)
          )
        val delay = DateUtils.getTimeUnit(Try(parameter.get(KEY_FLINK_RESTART_STRATEGY_FIXED_DELAY_DELAY))
          .getOrElse(
            Try(defaultFlinkConf(KEY_FLINK_RESTART_STRATEGY_FIXED_DELAY_DELAY.drop(prefixLen)))
              .getOrElse(null))
          )
  ...
  private[this] def checkpoint(): Unit = {
    ...
    val stateBackendValue = Option(parameter.get(KEY_FLINK_STATE_BACKEND))
    if (stateBackendValue.isDefined) {
      val stateBackend = XStateBackend.withName(stateBackendValue.get)
      require(
      ...
      val storage = {
        val storage = Option(parameter.get(KEY_FLINK_STATE_CHECKPOINT_STORAGE)) match {
          case None =>
      ...
      lazy val cpDir = Option(parameter.get(KEY_FLINK_STATE_CHECKPOINTS_DIR)) match {
        case None =>
      ...
      stateBackend match {
        case XStateBackend.hashmap =>
          ...
          storage match {
            case CheckpointStorage.jobmanager =>
              ...
              val maxMemorySize = Try(parameter.get(KEY_FLINK_STATE_BACKEND_MEMORY).toInt)
                .getOrElse(JobManagerCheckpointStorage.DEFAULT_MAX_STATE_SIZE)
              cpConfig.setCheckpointStorage(new JobManagerCheckpointStorage(maxMemorySize))
        ...
        case XStateBackend.rocksdb =>
          ...
          parameter.getProperties
            ...
            .filterNot(x => skipKey.contains(x._1)).foreach(x => map.put(x._1, x._2))
          if (map.nonEmpty) {
