 ...
 *
 * See: [[org.apache.streampark.shaded.org.apache.flink.kubernetes.operator.api.FlinkDeployment]]
 */
    ...
    watchK8sResourceForever(client =>
      ...
      stream
        ...
        .map { case (action, deployment) =>
          DeployCRStatus.eval(action, deployment) -> Option(deployment.getStatus.getJobStatus).map(JobStatus.fromCR)
        }
