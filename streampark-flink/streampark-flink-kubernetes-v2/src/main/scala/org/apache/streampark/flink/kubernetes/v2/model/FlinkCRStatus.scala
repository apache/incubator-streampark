...
import org.apache.streampark.shaded.org.apache.flink.kubernetes.operator.api.lifecycle.ResourceLifecycleState
import org.apache.streampark.shaded.org.apache.flink.kubernetes.operator.api.status.JobManagerDeploymentStatus
...
/**
 * Flink deployment CR status. See:
 * [[org.apache.streampark.shaded.org.apache.flink.kubernetes.operator.api.FlinkDeployment]]
 */
...
object DeployCRStatus {
  ...
  def eval(action: Watcher.Action, cr: FlinkDeployment): DeployCRStatus = {
    ...
    val evalState = (action, lifecycle, jmDeployStatus) match {
      ...
      case (_, ResourceLifecycleState.FAILED, _)    => EvalState.FAILED
      case (_, _, JobManagerDeploymentStatus.ERROR) => EvalState.FAILED
      case (_, ResourceLifecycleState.SUSPENDED, _) => EvalState.SUSPENDED
...
/**
 * Flink Session Job CR status. See:
 * [[org.apache.streampark.shaded.org.apache.flink.kubernetes.operator.api.FlinkSessionJob]]
 */
...
object SessionJobCRStatus {
  ...
  def eval(action: Watcher.Action, cr: FlinkSessionJob): SessionJobCRStatus = {
    ...
    val evalState = (action, lifecycle) match {
      ...
      case (_, ResourceLifecycleState.STABLE)      => EvalState.READY
      case (_, ResourceLifecycleState.ROLLED_BACK) => EvalState.READY
      case (_, ResourceLifecycleState.SUSPENDED)   => EvalState.SUSPENDED
      case (_, ResourceLifecycleState.FAILED)      => EvalState.FAILED
      case _                                       => EvalState.DEPLOYING
    }
