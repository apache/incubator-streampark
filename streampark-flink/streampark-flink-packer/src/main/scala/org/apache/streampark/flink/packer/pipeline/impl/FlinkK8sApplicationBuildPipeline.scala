...
class FlinkK8sApplicationBuildPipeline(request: FlinkK8sApplicationBuildRequest)
  ...
  override protected def buildProcess(): DockerImageBuildResponse = {
    ...
    if (request.isImageResource) {
      val mainJarPath = request.customFlinkUserJar.trim match {
        case jar if jar.startsWith("local://") =>
          s"local://$jar"
        case jar =>
          if (!jar.startsWith("/")) {
            throw new RuntimeException("main jar path must start form local:/// or /")
          }
          jar
      }
