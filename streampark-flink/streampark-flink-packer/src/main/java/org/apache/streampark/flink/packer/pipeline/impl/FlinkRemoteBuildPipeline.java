...
public class FlinkRemoteBuildPipeline extends BuildPipeline {
    ...
    public FlinkRemoteBuildPipeline(FlinkRemotePerJobBuildRequest request) { this.request = request; }
    public static FlinkRemoteBuildPipeline of(FlinkRemotePerJobBuildRequest request) { return new FlinkRemoteBuildPipeline(request); }

    @Override public PipelineTypeEnum getPipeType() { return PipelineTypeEnum.FLINK_STANDALONE; }
    ...
    @Override protected BuildResult buildProcess() throws Throwable {
        ...
        execStep(1, () -> { LfsOperator.getInstance().mkCleanDirs(request.workspace()); return null; })
            .orElseThrow(() -> getError().exception());
        File shadedJar = execStep(2, () -> {
            if (request.flinkJobType() == FlinkJobType.FLINK_SQL) {
                return MavenTool.buildFatJar(request.mainClass(), request.providedLibs(), request.getShadedJarPath(request.workspace()));
            }
        ...
        execStep(4, () -> {
            if (request.flinkJobType() == FlinkJobType.PYFLINK) {
                for (String jar : mavenJars) {
                    ...
                    if (originFile.isFile()) lfs.copy(originFile.getAbsolutePath(), lib);
                }
            }
            return null;
        }).orElseThrow(() -> getError().exception());
