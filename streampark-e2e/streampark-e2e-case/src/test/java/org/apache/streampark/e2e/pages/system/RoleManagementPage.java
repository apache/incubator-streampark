...
public class RoleManagementPage extends NavBarPage implements SystemPage.Tab {
  @FindBy(
      xpath =
          "//span[contains(., 'Role List')]/..//button[contains(@class, 'ant-btn-primary')]/span[contains(text(), 'Add New')]")
  ...
  private void editRoleMenu(String menuName) {
      createRoleForm.inputMenus.stream()
          .filter(e ->
              Objects.equals(
                  e.findElement(By.xpath(".//span[contains(@class, 'streampark-tree__title') and contains(@class, 'pl-2')]")).getText(), menuName))
          .findFirst()
          .orElseThrow(
              () ->
