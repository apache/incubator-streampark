...
docker build -t streamx/mysql:$version mysql
...
wget  --no-check-certificate -O streamx.gz   https://github.com/streamxhub/streamx/releases/download/$1/streamx-console-service-${v_num}-release.gz
mv -f streamx.gz ./streamx/
docker build --build-arg streamx_version=$version -t streamx/streamx-console-service:$version  streamx
